...
async def handle_user_reply_ocr_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cached:
        ...
        if remaining <= 0:
            await warn(update, "Daily OCR Limit Reached", f"Your daily OCR limit is over for today.\nTomorrow you can use it again.\nDaily limit: <code>{h(str(get_mistral_user_daily_limit()))}</code>")
            raise ApplicationHandlerStop
...
# =========================================================
# ===== FINAL COMPREHENSIVE FIX PATCH (2026-04-12) =====
# Fixes:
...
# 4. Group quote-style message suppression
# 5. Telegram Streaming (live message update) for AI responses
...
# ── Re-register updated build_app to include all fixes ──
_prev_build_app_final_fix = build_app

def build_app() -> Application:
    app = _prev_build_app_final_fix()

    # Re-register the updated handlers (higher priority group = -20)
    private_filter = filters.ChatType.PRIVATE
    with contextlib.suppress(Exception):
    ...
    with contextlib.suppress(Exception):
        # Updated solver callback
        app.add_handler(
            CallbackQueryHandler(on_solver_callback, pattern=r"^solve:[GPD]:[0-9a-f]{6,16}$"),
            group=-20,
        )

    logger.info("[FINAL FIX PATCH 2026-04-12] All fixes applied successfully.")
    return app


logger.info("[FINAL FIX PATCH 2026-04-12] Patch loaded: quiz fix, key rotation, streaming, reply flow, group quote fix.")
