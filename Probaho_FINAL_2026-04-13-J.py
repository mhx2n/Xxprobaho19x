...
# ===== END PROBAHO PATCH 2026-04-13-J =====
...
# ═══════════════════════════════════════════════════════════════════════════
# ===== PROBAHO FINAL PATCH-J (2026-04-13) =====
#
# নতুন সংযোজন ও সমস্যা সমাধান:
# 1. Gemini multi-key DB rotation: model-level failover
#    (2.0-flash → 2.5-flash → gemini-3-flash → Gemini3 web → Perplexity)
# 2. Admin OCR → quiz buffer: robust extraction, no JSON errors
# 3. /qans + /pans: smart question targeting for admin & user both
# 4. /gen: fix for admin unlimited, user 10/page 30/day quota
# 5. Quiz verify: majority-vote across 2 solvers (not 3 slow ones)
# 6. /gemini status: shows active key + model rotation
# 7. Safe_reply group: no quote (retained from master patch)
# ═══════════════════════════════════════════════════════════════════════════

# ── Gemini model rotation list (all 3 generations) ──
_GEMINI_TEXT_MODELS_J = [
    "models/gemini-2.0-flash",
    "models/gemini-2.5-flash",
    "models/gemini-3-flash",
    "models/gemini-2.0-flash-lite",
]
_GEMINI_VISION_MODELS_J = [
    "models/gemini-2.0-flash",
    "models/gemini-2.5-flash",
    "models/gemini-3-flash",
]
...
def _all_text_model_candidates() -> List[str]:  # noqa: F811
    """Return all text models to try, deduped."""
    seen: set = set()
    out: List[str] = []
    for m in _GEMINI_TEXT_MODELS_J:
        nm = _normalize_model_name(m)
        if nm and nm not in seen:
            seen.add(nm)
            out.append(nm)
    # Also add from env if set
    for raw in re.split(r"[\s,;]+", os.getenv("GEMINI_TEXT_MODELS", "")):
        nm = _normalize_model_name(raw.strip())
        if nm and nm not in seen:
            seen.add(nm)
            out.append(nm)
    return out or ["models/gemini-2.0-flash"]
...
def _all_vision_model_candidates() -> List[str]:  # noqa: F811
    seen: set = set()
    out: List[str] = []
    for m in _GEMINI_VISION_MODELS_J:
        nm = _normalize_model_name(m)
        if nm and nm not in seen:
            seen.add(nm)
            out.append(nm)
...
# ── Ultra-resilient Gemini call: rotate models × keys ──
def _gemini_call_rotate_all(payload_fn, *, timeout_seconds: int = 20, force_json: bool = False) -> str:
    """
    Try every (model, key) combination until one succeeds.
    payload_fn(model) → dict  (builds the request payload for a given model)
    Returns the text response.
    """
    keys = get_gemini_api_keys()
    if not keys:
        raise RuntimeError("No Gemini API key. Use /gemini add YOUR_KEY first.")
    ...
    for model in models:
        for api_key in keys:
            url = (
                ...
                f"{_normalize_model_name(model)}:generateContent?key={api_key}"
            )
            try:
                ...
                _gemini_mark_key_status(api_key, "error", str(e)[:120])
                last_err = e
                continue

    raise RuntimeError(str(last_err or "All Gemini model+key combinations failed."))


def call_gemini_text_rest(prompt: str, timeout_seconds: int = 20, *, force_json: bool = False) -> str:  # noqa: F811
    """PATCH-J: rotate all models × all keys; fallback Gemini3 web."""
    def _payload(model: str) -> Dict[str, Any]:
        return _build_gemini_text_payload(prompt, force_json=force_json)

    # Try REST rotation first
    try:
        return _gemini_call_rotate_all(_payload, timeout_seconds=timeout_seconds, force_json=force_json)
    except Exception as rest_err:
        pass

    # Fallback: Gemini3 web session
    try:
        out = gemini3_solve(prompt)
        if out and str(out).strip():
            return str(out).strip()
    ...
    raise RuntimeError("All Gemini text backends unavailable. Please try again.")


def call_gemini_vision_rest(image_path: str, prompt: str, force_json: bool = True) -> str:  # noqa: F811
    """PATCH-J: rotate vision models × all keys."""
    keys = get_gemini_api_keys()
    if not keys:
        raise RuntimeError("No Gemini API key. Use /gemini add YOUR_KEY first.")
    ...
    for model in models:
        for api_key in keys:
            ...
            try:
                payload = _build_gemini_vision_payload(image_path, prompt, force_json=force_json)
                r = _requests_with_retries(
                    requests.post, url,
                    json_payload=payload,
                    timeout=30,
    ...
    raise RuntimeError(str(last_err or "All vision backends unavailable."))


# ── Core solve text/MCQ - always tries all backends ──
def _j_solve_text(prompt: str) -> Tuple[str, str]:
    """PATCH-J: Gemini REST (all models+keys) → Gemini3 web → Perplexity."""
    # 1. Gemini REST rotation
    try:
        out = call_gemini_text_rest(prompt, timeout_seconds=20)
        if out and out.strip():
            return _sanitize_answer_text(out.strip()), "✨ Gemini"
    except Exception:
        pass
    ...
    raise RuntimeError("All AI backends temporarily unavailable. Please try again in a moment.")


def _j_solve_mcq(question: str, options: List[str]) -> Tuple[Dict[str, Any], str]:
    """PATCH-J: Gemini REST → Gemini3 web → Perplexity for MCQ."""
    q = str(question or "").strip()
    ...
    prompt = (
        "Return STRICT JSON only. No markdown. No extra text.\n"
        "Solve the MCQ and pick the single correct answer.\n"
        "JSON:\n"
        f'{{"answer":1,"confidence":85,"explanation":"{schema_expl}","why_not":{{"A":"..","B":"..","C":"..","D":".."}}}}\n\n'
        f"Question:\n{q}\n\nOptions:\n{opt_lines}\n"
    )
    ...
    if USE_PERPLEXITY_FALLBACK:
        try:
            ...
            if alt:
                ...
                if isinstance(data, dict) and int(data.get("answer", 0) or 0) > 0:
                    return data, "⚛ Perplexity"
                return {"answer": 0, "confidence": 0, "explanation": str(alt)[:1800], "why_not": {}}, "⚛ Perplexity"
        except Exception as e:
            last_err = e

    raise RuntimeError(f"AI backends unavailable. ({str(last_err)[:80]})")


# Override the routing functions
def _solve_text_via_prompt(prompt: str, preferred: str = "G") -> Tuple[str, str]:  # noqa: F811
    code = (preferred or "G").upper()
    if code == "P":
        try:
            alt = query_ai(prompt)
...
def _solve_mcq_with_preference(model: str, question: str, options: List[str]) -> Tuple[Dict[str, Any], str]:  # noqa: F811
    ...
    if code == "P":
        try:
            ...
            pass
        return _j_solve_mcq(question, options)
    return _j_solve_mcq(question, options)


def gemini_solve_text(problem_text: str) -> str:  # noqa: F811
    prompt = STRICT_SYSTEM_PROMPT + "\n\nUser Message:\n" + str(problem_text or "").strip()
    out, _ = _j_solve_text(prompt)
    return out


def gemini_solve_mcq_json(question: str, options: List[str]) -> Dict[str, Any]:  # noqa: F811
    data, _ = _j_solve_mcq(question, options)
    return data


# ── PATCH-J: Robust quiz generation ──
def _j_generate_quizzes_from_ocr(ocr_ctx: Dict[str, Any], desired: int, user_id: int) -> List[Dict[str, Any]]:
    """
    PATCH-J: Robust quiz generation with:
    - Gemini REST (all models) → Gemini3 web → Perplexity fallback
    - Fast 2-solver verify (not slow 3-solver)
    - Proper JSON parse with repair
    """
    source_text = _build_ocr_quiz_source(ocr_ctx)
    ...
    # Try in chunks of up to 5
    batch_size = min(5, desired)
    attempts = 0
    max_attempts = (desired // batch_size + 1) * 3  # generous attempts
    ...
    while len(out) < desired and attempts < max_attempts:
        ...
        prompt = (
            ...
            f"Generate exactly {need} NEW original MCQ quiz questions based on the page below.\n"
            "Rules:\n"
            "- 4 options per question. Exactly 1 correct answer (answer: integer 1-4).\n"
            "- Match the language of the source page (Bangla if source is Bangla).\n"
            "- Short accurate explanation (1-2 lines).\n"
            "- Do NOT copy questions verbatim from source.\n"
            "Output ONLY:\n"
            '{"items":[{"question":"...","options":["opt1","opt2","opt3","opt4"],"answer":1,"explanation":"..."}]}\n\n'
            f"PAGE CONTENT:\n{source_text[:14000]}{avoid_note}"
        )

        raw = None
        # Backend chain
        for fn in [
            lambda: call_gemini_text_rest(prompt, timeout_seconds=25, force_json=True),
            lambda: gemini3_solve(prompt),
        ...
        for parse_fn in [
            ...
            lambda s: json.loads(re.search(r'\{.*"items".*\}', s, re.DOTALL).group(0)) if re.search(r'\{.*"items".*\}', s, re.DOTALL) else None,
            lambda s: _repair_to_json(s, schema_hint='{"items":[{"question":"...","options":["...","...","...","..."],"answer":1,"explanation":"..."}]}', timeout_seconds=12),
        ]:
            try:
                data = parse_fn(raw_s)
        ...
        for it in items_raw:
            ...
            # Fast 2-solver verify (Gemini + Perplexity only for speed)
            votes: List[int] = []
            expls: Dict[int, str] = {}
            for solver_fn in [gemini_solve_mcq_json, perplexity_solve_mcq_json]:
                try:
                    res = solver_fn(q, opts)
                    a = int(res.get("answer", 0) or 0)
                    if 1 <= a <= 4:
                        votes.append(a)
                        v_expl = str(res.get("explanation") or "").strip()
                        if v_expl and a not in expls:
                            expls[a] = v_expl
                except Exception:
                    pass
            ...
            if votes:
                ...
                best_ans = _Ctr(votes).most_common(1)[0][0]
                if expls.get(best_ans):
                    expl = expls[best_ans]
                tentative = best_ans

            out.append({"question": q, "options": opts, "answer": tentative, "explanation": expl or "See solution."})

    return out[:desired]


# Override
_generate_quizzes_from_ocr_sync = _j_generate_quizzes_from_ocr  # noqa: F811


# ── PATCH-J: /qans and /pans - smart for BOTH admin and user ──
# The admin /qans should also use the smart Q-number picker

def _j_smart_pick_mcq(items: List[Dict[str, Any]], user_text: str) -> Optional[Dict[str, Any]]:
    """Smart question picker - works for both admin and user."""
    pool = [dict(x) for x in (items or []) if str((x or {}).get("questions") or "").strip()]
    if not pool:
        return None
    ...
    # Free-form → return None to signal full-page context
    free_form_kw = re.compile(
        r"\b(summarize|summary|সারাংশ|সংক্ষেপ|explain|ব্যাখ্যা|বুঝাও|বুঝিয়ে|"
        r"analyze|analysis|বিশ্লেষণ|what is|describe|list|overview|কি আছে|কী কী)\b",
        re.IGNORECASE
    )
    _bd_tr = str.maketrans("০১২৩৪৫৬৭৮৯", "0123456789")
    t_norm = user_t.translate(_bd_tr)

    if free_form_kw.search(user_t):
        qno_chk = re.search(r'\b\d{1,3}\b', t_norm)
        if not qno_chk:
            return None
    ...
    for pat in [
        r"\b[Qq]\.?\s*(\d{1,3})\b",
        r"\b(\d{1,3})\s*(?:নম্বর|number|no\.?|th|st|nd|rd|নং)\b",
        r"(?:question|প্রশ্ন)\s*\.?\s*(\d{1,3})\b",
        r"\bNo\s*[\.\:\-]?\s*(\d{1,3})\b",
    ]:
    ...
    if not qno:
        stripped = t_norm.strip()
        if re.match(r"^\d{1,3}$", stripped):
            qno = stripped

    if qno:
        # exact match by _item_question_no
        exact = [it for it in pool if _item_question_no(it) == qno]
        if exact:
            return exact[0]
        # positional
        ...
        # text starts with number
        for it in pool:
            if re.match(rf"^\s*{re.escape(qno)}\s*[\.।\)]", str(it.get("questions") or "")):
                return it
...
def _j_build_ocr_answer_prompt(ocr_ctx: Dict[str, Any], user_question: str, previous_answer: str = "") -> str:
    """Build perfect OCR answer prompt for both admin and user."""
    user_q = str(user_question or "").strip()
    prev = str(previous_answer or "").strip()
    items = list((ocr_ctx or {}).get("items") or [])
    full_text = str((ocr_ctx or {}).get("clean_text") or (ocr_ctx or {}).get("raw_markdown") or "").strip()

    picked = _j_smart_pick_mcq(items, user_q)

    sys_pre = (
        "You are an expert academic AI for a Bangladeshi student platform.\n"
        "Answer ONLY from the provided page content.\n"
        "Language: Match the source (Bangla if source is Bangla).\n"
        "Format: Clean plain text. No LaTeX. No markdown headers.\n\n"
    )
    ...
    if picked:
        ...
        qblock = str(picked.get("questions") or "").strip()
        opt_block = "\n".join([f"{chr(64+i+1)}. {opts[i]}" for i in range(len(opts))])
        vis = int(picked.get("answer", 0) or 0)
        vis_txt = opts[vis - 1] if 1 <= vis <= len(opts) else ""
        qno = _item_question_no(picked) or ""
        label = f"Question {qno}" if qno else "Question"

        prompt = sys_pre + f"User request: {user_q}\n\n"
        if prev:
            prompt += f"Previous answer:\n{prev[:1500]}\n\n"
        prompt += f"--- {label} from page ---\n{qblock}\n\nOptions:\n{opt_block}\n"
        if vis_txt:
            prompt += f"\nMarked correct answer on page: {chr(64+vis)}) {vis_txt}\n"
        prompt += "\nProvide:\n1) Correct answer with option letter\n2) Step-by-step explanation\n3) Why others are wrong\n"
        return prompt
    else:
        prompt = sys_pre + f"User request about this page:\n{user_q}\n\n"
        if prev:
            prompt += f"Previous answer:\n{prev[:1500]}\n\n"
        prompt += f"--- Full page content ---\n{full_text[:12000]}\n"
        prompt += "\nAnswer the user's request comprehensively."
        return prompt


# Override _build_focused_ocr_prompt globally
_build_focused_ocr_prompt = _j_build_ocr_answer_prompt  # noqa: F811

# Override _pick_first_mcq_item globally
def _pick_first_mcq_item(items: List[Dict[str, Any]], extra_instruction: str = "") -> Optional[Dict[str, Any]]:  # noqa: F811
    result = _j_smart_pick_mcq(items, extra_instruction)
    if result is None and items:
        pool = [x for x in items if str((x or {}).get("questions") or "").strip()]
        return pool[0] if pool else None
    return result
...
# ── PATCH-J: Admin quiz buffer - robust extraction ──
def _j_buffer_quizzes_from_ocr(admin_id: int, items: List[Dict[str, Any]], clean_text: str) -> int:
    """
    PATCH-J: Robust quiz buffering for admin.
    Filters bad items, ensures valid options and answer.
    """
    added = 0
    for item in items:
        if not isinstance(item, dict):
        ...
        if not q:
            continue
        opts = [str(item.get(f"option{i}") or "").strip() for i in range(1, 6)
                if str(item.get(f"option{i}") or "").strip()]
        if len(opts) < 2:
            continue
        ans = int(item.get("answer", 0) or 0)
        if not (1 <= ans <= len(opts)):
            # Try to infer from answer text
            ans_text = str(item.get("answer_text") or "").strip().upper()
            if ans_text and ans_text[0] in "ABCDE":
                ans = ord(ans_text[0]) - 64
            else:
                ans = 0  # unknown, still buffer it
        ...
        try:
            buffer_add(admin_id, payload)
            added += 1
        except Exception as e:
            logger.warning("[BUFFER] Failed to add item: %s", e)

    return added


# ── PATCH-J: /gemini status shows model rotation ──
def _gemini_runtime_report_html() -> str:  # noqa: F811
    rows = _gemini_key_rows(include_disabled=True)
    active_rows = [r for r in rows if int(r.get("is_enabled") or 0) == 1]
    lines = [
        f"🔑 <b>Active keys</b>: <code>{len(active_rows)}</code> / <code>{len(rows)}</code>",
        f"📝 <b>Text models (in order)</b>: <code>{h(' → '.join(_all_text_model_candidates()[:4]))}</code>",
        f"🖼 <b>Vision models</b>: <code>{h(' → '.join(_all_vision_model_candidates()[:3]))}</code>",
        "",
    ]
    if active_rows:
        lines.append("<b>Key order:</b>")
        for r in active_rows[:10]:
            kid = str(r.get("id") or "env")
            mask = _normalize_secret_mask(str(r.get("api_key") or ""))
            label = str(r.get("label") or "")
            status = str(r.get("last_status") or "ready")
            lines.append(f"• <code>{h(kid)}</code> <code>{h(mask)}</code> {h(label)} — <i>{h(status)}</i>")
    else:
        lines.append("⚠️ No active Gemini keys. Use /gemini add YOUR_KEY")
    return "\n".join(lines)


# ── PATCH-J: User quota for /gen ──
_J_GEN_PAGE_LIMIT = 10   # user: per page per day
_J_GEN_DAY_LIMIT  = 30   # user: total per day


# ── PATCH-J: Final build_app registration ──
_prev_build_app_j = build_app

def build_app() -> Application:  # noqa: F811
    app = _prev_build_app_j()

    # Re-register /gen with updated logic
    with contextlib.suppress(Exception):
        _register_dual_command(app, "gen", cmd_gen, group=-200)

    # Re-register /pans (alias for smart qans)
    with contextlib.suppress(Exception):
        _register_dual_command(app, "pans", cmd_pans, group=-200)

    # Re-register /qans with higher priority
    if "cmd_qans" in globals():
        with contextlib.suppress(Exception):
            _register_dual_command(app, "qans", globals()["cmd_qans"], group=-200)

    logger.info("[PATCH-J] Registered: model×key rotation, /gen robust quiz, /qans smart picker, /pans alias.")
    return app


logger.info("[PATCH-J 2026-04-13] Loaded: Gemini 2.0+2.5+3-flash rotation, robust quiz gen, smart /qans+/pans, admin buffer fix.")

# ===== END PROBAHO FINAL PATCH-J =====
