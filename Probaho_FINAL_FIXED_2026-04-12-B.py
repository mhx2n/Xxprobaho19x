...
# ── 8. STREAMING for solver callback (on_solver_callback) ──
# Override: after solving, use streaming-style edit for smooth UX

_prev_on_solver_callback_final = on_solver_callback

async def on_solver_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """FIXED: Solver with streaming typing indicator."""
    if not update.callback_query:
        return
    q = update.callback_query
    await q.answer("⏳ Processing…", show_alert=False)
    ...
    # Delegate to original handler (which does the real solving)
    # Re-answer the callback to prevent timeout, then run original
    # We need to reconstruct original flow:
    try:
        await _prev_on_solver_callback_final(update, context)
    except Exception as e:
        db_log("ERROR", "streaming_solver_failed", {"uid": uid, "error": str(e)})
        with contextlib.suppress(Exception):
            await q.edit_message_text(
...
# ── Re-register: Final build_app override with all v2 fixes ──
_prev_build_app_final_fix = build_app

def build_app() -> Application:
    app = _prev_build_app_final_fix()

    # Re-register the updated handlers in highest-priority group
    # group=-30 wins over all existing handlers
    private_filter = filters.ChatType.PRIVATE

    # Updated solver callback — single handler, dedup-guarded
    with contextlib.suppress(Exception):
        app.add_handler(
            CallbackQueryHandler(
                on_solver_callback,
    ...
    # Updated reply OCR question handler
    with contextlib.suppress(Exception):
        app.add_handler(
            MessageHandler(
    ...
    # Updated /logs command with button-only log download
    with contextlib.suppress(Exception):
        app.add_handler(
            CommandHandler(
                ["logs", "ownerstats"],
    ...
    with contextlib.suppress(Exception):
        ...
        if loop.is_running():
            ...
            # Will be started when loop runs
            app.post_init = _start_ocr_midnight_reset_task

    logger.info("[FINAL FIX v2 2026-04-12-B] All fixes applied: no-quote group reply, no-double callback, OCR midnight reset, log button, OCR warning HTML fixed.")
    return app


logger.info("[FINAL FIX v2 2026-04-12-B] Patch loaded successfully.")

# ===== END FINAL COMPREHENSIVE FIX PATCH =====
