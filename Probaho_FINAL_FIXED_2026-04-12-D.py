...
# =========================================================
# ===== FINAL COMPREHENSIVE FIX PATCH v2 (2026-04-12-B) =====
# Fixes:
# 1. Quiz generation from image - full multi-question extraction
# 2. Specific question targeting (Q8, Q3 etc.) from any page
# 3. Reply-to-bot-response flow continuity
# 4. Group quote-style message suppression (FIXED - no quoted reply)
# 5. Telegram Streaming for AI responses (FIXED - no duplicate processing)
# 6. Mistral multi-key: proper rotation when one exhausted
# 7. [NEW] Double-processing prevention for solver callback
# 8. [NEW] OCR limit reset at midnight (Bangladesh time, 00:00 BST)
# 9. [NEW] OCR limit warning message - proper Telegram HTML (no raw <code> tags)
# 10. [NEW] Owner /logs command: stats message + inline "Download Log" button
# 11. [NEW] "Processing..." spinner removed from group solver
# =========================================================

import asyncio as _asyncio_fix

# ═══════════════════════════════════════════════════════════════
# FIX-A: OCR LIMIT WARNING — সঠিক Telegram HTML (raw tag নেই)
# ═══════════════════════════════════════════════════════════════

def _ocr_limit_warning_html(daily_limit: int) -> str:
    return (
        "⚠️ <b>Daily OCR Limit Reached</b>\n"
        "Your daily OCR limit is over for today.\n"
        "Tomorrow you can use it again.\n"
        f"Daily limit: <b>{daily_limit}</b>"
    )

async def _send_ocr_limit_warning(update: Update, daily_limit: int) -> None:
    """Send OCR limit exceeded message - proper HTML, no raw tags."""
    msg = _ocr_limit_warning_html(daily_limit)
    if update.message:
        with contextlib.suppress(Exception):
            await update.message.reply_text(
                msg,
...
# ═══════════════════════════════════════════════════════════════
# FIX-B: OCR LIMIT MIDNIGHT RESET (Bangladesh Time = UTC+6)
# প্রতিদিন রাত ১২টায় (BST) নতুন day_key তৈরি হয়, পুরানো entries clean করে
# ═══════════════════════════════════════════════════════════════

_ocr_midnight_reset_started = False

async def _ocr_midnight_reset_loop() -> None:
    """Background task: waits until next midnight BST, then loops daily."""
    global _ocr_midnight_reset_started
    _BD_TZ_RESET = dt.timezone(dt.timedelta(hours=6))
    while True:
        try:
            now_bd = dt.datetime.now(_BD_TZ_RESET)
            # Next midnight
            next_midnight = (now_bd + dt.timedelta(days=1)).replace(
                hour=0, minute=0, second=5, microsecond=0
            )
            wait_seconds = (next_midnight - now_bd).total_seconds()
            await asyncio.sleep(max(wait_seconds, 1))

            # Clean up old day_key entries (keep only last 3 days)
            try:
                cutoff_date = (dt.datetime.now(_BD_TZ_RESET) - dt.timedelta(days=3)).strftime("%Y-%m-%d")
                conn = db_connect()
                cur = conn.cursor()
                cur.execute(
                    "DELETE FROM user_ocr_daily_usage WHERE day_key < ?",
                    (cutoff_date,)
                )
                conn.commit()
                conn.close()
                logger.info("[OCR RESET] Old OCR usage records cleaned. New day: %s", _today_bd_key())
            except Exception as e:
                logger.warning("[OCR RESET] Cleanup error: %s", e)

        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.warning("[OCR RESET] Loop error: %s", e)
            await asyncio.sleep(60)


def _start_ocr_midnight_reset_task() -> None:
    global _ocr_midnight_reset_started
    if _ocr_midnight_reset_started:
        return
    _ocr_midnight_reset_started = True
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            loop.create_task(_ocr_midnight_reset_loop())
            logger.info("[OCR RESET] Midnight reset task started.")
    except Exception as e:
        logger.warning("[OCR RESET] Could not start reset task: %s", e)


# ═══════════════════════════════════════════════════════════════
# FIX-C: GROUP REPLY — quote ছাড়া সাধারণ message (সব জায়গায়)
# ═══════════════════════════════════════════════════════════════

_orig_safe_reply_v2 = safe_reply

async def safe_reply(update: Update, text: str, **kwargs) -> Optional[Any]:
    """FIXED v2: Group-এ কোনো quote নেই — সাধারণ message পাঠায়।"""
    try:
        if not update or not update.message:
            return await _orig_safe_reply_v2(update, text, **kwargs)
        chat_type = str(getattr(update.effective_chat, "type", "") or "")
        if chat_type in ("group", "supergroup"):
            parts = chunk_text(text, 4000)
            last = None
            for part in parts:
                with contextlib.suppress(Exception):
                    last = await update.message.chat.send_message(
                        text=part,
                        parse_mode=ParseMode.HTML,
                        disable_web_page_preview=True,
                    )
...
# ═══════════════════════════════════════════════════════════════
# FIX-D: GROUP _reply_group_ai_direct — quote ছাড়া, Processing বাদ
# ═══════════════════════════════════════════════════════════════

async def _reply_group_ai_direct(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    prompt_text: str,
    scope: str = "group_general"
) -> None:
    """FIXED: Group AI reply - no quote, no 'Processing...' header, streaming effect."""
    if not update.message or not update.effective_chat:
        return
    ...
    # Send placeholder WITHOUT quoting (no reply_to_message_id)
    placeholder = None
    try:
    ...
    try:
        answer, backend_used = await _run_blocking(
            _role_of(uid), _solve_text_with_preference, "G", prompt_text, scope
        )
        if _contains_adult_content(answer):
            answer = _adult_refusal_text(prompt_text)
        preserve_code = looks_like_programming_request(prompt_text) or looks_like_programming_request(answer)
        ...
        db_log("INFO", "group_text_backend", {"user_id": uid, "backend": backend_used})
    except Exception as e:
        db_log("ERROR", "group_text_ai_failed", {"user_id": uid, "error": str(e)})
        fail_html = "❌ AI এখন ব্যস্ত। কিছুক্ষণ পরে আবার চেষ্টা করুন।"
        with contextlib.suppress(Exception):
            if placeholder:
                await context.bot.edit_message_text(
                    chat_id=chat_id,
        ...
        msg_id = placeholder.message_id if placeholder else None
        if msg_id:
            asyncio.create_task(
                _auto_delete_after(context.bot, chat_id, [msg_id], GROUP_BOT_MESSAGE_TTL_SECONDS)
            )


# ═══════════════════════════════════════════════════════════════
# FIX-E: SOLVER CALLBACK — double processing বন্ধ, একটাই handler
# Processing… popup বাদ, streaming UX চালু
# ═══════════════════════════════════════════════════════════════

# Track answered callbacks to prevent double-fire
_answered_callbacks: set = set()
_MAX_ANSWERED_CACHE = 2000

async def on_solver_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """FINAL FIXED: Single-execution solver callback. No double processing. No Processing popup."""
    if not update.callback_query:
        return
    q = update.callback_query
    ...
    # ── Deduplication guard ──
    cb_id = str(q.id or "")
    if cb_id and cb_id in _answered_callbacks:
        with contextlib.suppress(Exception):
            await q.answer()
        return
    if cb_id:
        _answered_callbacks.add(cb_id)
        if len(_answered_callbacks) > _MAX_ANSWERED_CACHE:
            # Keep only last half to avoid memory leak
            items = list(_answered_callbacks)
            _answered_callbacks.clear()
            _answered_callbacks.update(items[_MAX_ANSWERED_CACHE // 2:])

    # Answer callback silently (no popup text)
    with contextlib.suppress(Exception):
        await q.answer()
    ...
    if not isinstance(req, dict):
        with contextlib.suppress(Exception):
            await q.edit_message_text("⚠️ এই request মেয়াদোত্তীর্ণ। আবার প্রশ্ন পাঠান।")
        return

    uid = int(req.get("uid") or 0)
    if q.from_user and q.from_user.id != uid:
        with contextlib.suppress(Exception):
            await q.answer("এটি আপনার request নয়।", show_alert=True)
        return
    ...
    # Show streaming "thinking" indicator (edit message)
    with contextlib.suppress(Exception):
        await q.edit_message_text(
            "⏳ <i>AI উত্তর তৈরি করছে...</i>",
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True,
        )
    ...
    try:
        public_name = _public_model_name(model, "AI") if callable(getattr(__builtins__ if hasattr(__builtins__, 'get') else {}, '_public_model_name', None)) else (
            "Gemini" if model == "G" else "Perplexity" if model == "P" else "DeepSeek"
        )
        # Use the correct public name function
        try:
            public_name = _public_model_name(model, "AI")
        except Exception:
            public_name = {"G": "Gemini", "P": "Perplexity", "D": "DeepSeek"}.get(model, "AI")
        ...
        with contextlib.suppress(Exception):
            ...
            if q.message and kind == "poll":
                _remember_quiz_context(context, q.message.message_id, payload)
            if is_group and q.message:
                asyncio.create_task(
                    _auto_delete_after(context.bot, q.message.chat_id, [q.message.message_id], GROUP_BOT_MESSAGE_TTL_SECONDS)
                )

    except Exception as e:
        db_log("ERROR", "solver_callback_v2_failed", {"user_id": uid, "model": model, "error": str(e)})
        with contextlib.suppress(Exception):
            await q.edit_message_text(
...
# ═══════════════════════════════════════════════════════════════
# FIX-F: OWNER /logs COMMAND — stats message আলাদা, log file শুধু
#         "Download Log" button ক্লিক করলে পাঠাবে
# ═══════════════════════════════════════════════════════════════

_LOG_DOWNLOAD_CB_PREFIX = "owner_dl_log:"

@require_owner
async def cmd_ownerstats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner /logs — Stats দেখায়। Log file দেয় শুধু button ক্লিকে।"""
    conn = db_connect()
    cur = conn.cursor()
    ...
    since_active_dt = dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=24)
    since_active_iso = since_active_dt.replace(microsecond=0).isoformat()
    cur.execute(
    ...
    since_error_dt = dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=1)
    since_error_iso = since_error_dt.replace(microsecond=0).isoformat()
    cur.execute(
        "SELECT COUNT(*) AS c FROM bot_logs WHERE level='ERROR' AND created_at >= ?",
        (since_error_iso,)
    )
    err_1h = int(cur.fetchone()["c"] or 0)

    cur.execute(
        "SELECT created_at, event, meta_json FROM bot_logs WHERE level='ERROR' ORDER BY id DESC LIMIT 5"
    )
    last_errors = cur.fetchall()
    conn.close()
    ...
    lines = [
        ...
        f"⏱ Uptime: <code>{h(fmt_uptime())}</code>",
        "",
        f"👥 Total Users: <b>{h(str(total_users))}</b>",
        f"🛠 Staff Accounts: <b>{h(str(staff_count))}</b>",
        f"✅ Active Users (24h): <b>{h(str(active_24h))}</b>",
        "",
        f"💾 Database Size: <code>{h(fmt_mb(db_mb))}</code>",
        f"🧠 RAM (RSS): <code>{h(fmt_mb(rss_mb))}</code>",
        f"☁️ GitHub Backup: <code>{h(github_status)}</code>",
        "",
        f"🔴 Errors (Last 1 Hour): <b>{h(str(err_1h))}</b>",
    ]
    ...
    if last_errors:
        ...
        for row in last_errors:
            ...
            try:
                ...
                meta = ""
            meta_disp = h(meta.replace("\n", " ")[:60]) if meta else ""
            if meta_disp:
                lines.append(f"• <code>{h(ts)}</code> — {h(event)} — <i>{meta_disp}</i>")
            else:
                lines.append(f"• <code>{h(ts)}</code> — {h(event)}")

    # Log download button (একটাই button — ক্লিক করলে file দেবে)
    req_id = uuid.uuid4().hex[:8]
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("📥 Download Log File", callback_data=f"{_LOG_DOWNLOAD_CB_PREFIX}{req_id}")
    ]])
...
async def _on_log_download_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Log file download button callback — owner only."""
    if not update.callback_query:
        return
    q = update.callback_query
    with contextlib.suppress(Exception):
        await q.answer("📥 লগ ফাইল তৈরি করা হচ্ছে...")

    uid = q.from_user.id if q.from_user else 0
    if not _is_owner_id(uid):
        with contextlib.suppress(Exception):
            await q.answer("শুধু Owner এটি ব্যবহার করতে পারবেন।", show_alert=True)
        return

    # Remove button to prevent re-click
    with contextlib.suppress(Exception):
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            await context.bot.send_message(
                chat_id=uid,
                text=f"❌ Log file তৈরি করতে ব্যর্থ: {h(str(e)[:200])}",
                parse_mode=ParseMode.HTML,
            )
    finally:
        if snapshot_path:
...
# ── 1. MISTRAL KEY: Smarter rotation - mark exhausted keys and skip them automatically ──
_orig_mistral_ocr_with_failover = _mistral_ocr_with_failover

def _mistral_ocr_with_failover(path: str) -> Dict[str, Any]:
    """Enhanced Mistral OCR with proper key rotation and auto-disable on limit."""
    keys = get_mistral_api_keys()
    if not keys:
        single = str(os.getenv("MISTRAL_API_KEY", "") or "").strip()
        if single:
    ...
    for key_info in keys:
        ...
        # Skip keys that were recently marked as limit-exhausted (auto-rotate)
        last_status = str(key_info.get("last_status") or "").strip().lower()
        if last_status in ("limit", "auth") and key_id > 0:
            # Re-check: only skip if fail_count is high (exhausted)
            conn = db_connect()
            cur = conn.cursor()
            cur.execute("SELECT fail_count, updated_at FROM mistral_api_keys WHERE id=?", (key_id,))
            row = cur.fetchone()
            conn.close()
            if row:
                fail_count = int(row["fail_count"] or 0)
                updated_at = str(row["updated_at"] or "")
                # If failed more than 2 times and updated within last 30 minutes, skip
                try:
                    from datetime import datetime, timezone, timedelta
                    upd = datetime.fromisoformat(updated_at.replace("Z", "+00:00"))
                    age_minutes = (datetime.now(timezone.utc) - upd).total_seconds() / 60
                    if fail_count >= 2 and age_minutes < 30:
                        limit_failures.append(_mask_secret(api_key))
                        last_error = RuntimeError(f"Key {_mask_secret(api_key)} is rate-limited, rotating...")
                        continue
                except Exception:
                    pass
        ...
        try:
            ...
            if mime.startswith("image/") and size_bytes <= 6 * 1024 * 1024:
                ...
                ok_up, up_res = _mistral_upload_file_for_key(path, api_key)
                if not ok_up:
                    status_code, body = up_res
                    kind = _mistral_error_kind(int(status_code), str(body))
                    _mistral_mark_key_status(key_id, kind, f"upload: {body}")
                    last_error = RuntimeError(f"upload failed ({status_code}): {str(body)[:100]}")
            ...
            if resp.status_code != 200:
                kind = _mistral_error_kind(resp.status_code, resp.text)
                _mistral_mark_key_status(key_id, kind, resp.text[:260])
                last_error = RuntimeError(f"OCR failed HTTP {resp.status_code}")
                if kind in ("limit", "auth"):
                    limit_failures.append(_mask_secret(api_key))
                    continue
                continue
            ...
            data = resp.json()
            if not isinstance(data, dict):
                _mistral_mark_key_status(key_id, "bad_response", "invalid JSON")
                last_error = RuntimeError("Mistral OCR returned invalid response.")
                continue

            pages = data.get("pages", []) or []
            chunks = [str((p or {}).get("markdown") or "").strip() for p in pages]
    ...
    if limit_failures and tried_keys == len(limit_failures):
        raise RuntimeError(
            f"All {len(limit_failures)} active Mistral key(s) are rate-limited. "
            "Add a new key with /mistral add YOUR_KEY or wait ~30 minutes."
        )
    raise RuntimeError(str(last_error or "Mistral OCR failed for all keys."))


# ── 2. IMPROVED QUIZ GENERATION - stronger, error-tolerant, full page extraction ──

def _build_robust_quiz_prompt(seed_question: str, seed_options: List[str]) -> str:
    """Build a very explicit quiz generation prompt to avoid errors."""
    sq = str(seed_question or "").strip()
    so = _normalize_options(seed_options or [], max_n=4)
    is_bn = _is_bangla_text(sq + " " + " ".join(so))
    lang = "Bangla" if is_bn else "English"
    schema_expl = "বাংলা ব্যাখ্যা এখানে" if is_bn else "Explanation here"

    return (
        "Return STRICT JSON ONLY. No markdown. No explanation outside JSON. No preamble.\n"
        f"Generate exactly 3 NEW MCQs on the SAME micro-topic as the seed question.\n"
        f"Language: {lang}. Do NOT repeat the seed question.\n"
        "Each MCQ must have exactly 4 options and one correct answer (answer: 1-4).\n"
        "Keep explanations to 1-2 sentences max.\n\n"
        "Required JSON format (output ONLY this, nothing else):\n"
        '{"items":['
        '{"question":"...","options":["A text","B text","C text","D text"],"answer":1,"explanation":"' + schema_expl + '"},'
        '{"question":"...","options":["A text","B text","C text","D text"],"answer":2,"explanation":"' + schema_expl + '"},'
        '{"question":"...","options":["A text","B text","C text","D text"],"answer":3,"explanation":"' + schema_expl + '"}'
        "]}\n\n"
        f"Seed Question: {sq}\n"
        "Seed Options:\n" + "\n".join([f"{i+1}. {so[i]}" for i in range(len(so))])
    )


def generate_quiz_items_gemini_then_verify(seed_question: str, seed_options: List[str]) -> List[Dict[str, Any]]:
    """
    FIXED: Generate 3 MCQs. Uses multiple backends with strong JSON enforcement.
    Verifies each answer with Perplexity fallback.
    """
    sq = str(seed_question or "").strip()
    so = _normalize_options(seed_options or [], max_n=4)

    prompt = _build_robust_quiz_prompt(sq, so)
    raw = None
    last_err = None
    ...
    backends = []
    if USE_GEMINI_REST_FOR_GENQUIZ and GEMINI_API_KEYS:
        backends.append(("gemini_rest", lambda: call_gemini_text_rest(prompt, timeout_seconds=25, force_json=True)))
    if USE_PERPLEXITY_FALLBACK:
        backends.append(("perplexity", lambda: query_ai(prompt)))
    backends.append(("gemini3", lambda: gemini3_solve(prompt)))
    ...
    if not raw:
        raise RuntimeError(f"Quiz generation failed - all backends unavailable: {last_err}")
    ...
    try:
        data = _extract_json_strict(raw_stripped)
    except Exception:
        pass
    ...
    if not isinstance(data, dict):
        try:
            m = re.search(r'\{.*"items"\s*:\s*\[.*?\]\s*\}', raw_stripped, re.DOTALL)
            if m:
    ...
    if not isinstance(data, dict):
        try:
            ...
            repaired = _repair_to_json(raw_stripped, schema_hint=schema_hint, timeout_seconds=15)
            if repaired:
                data = repaired
        except Exception:
            pass
    ...
    if not isinstance(data, dict):
        raise RuntimeError(f"Quiz generation: could not parse JSON from AI response.")
    ...
    for it in items_raw[:6]:  # allow up to 6 candidates, keep best 3
        if not isinstance(it, dict):
            continue
        q = str(it.get("question") or it.get("questions") or "").strip()
        if not q:
            continue
        ...
        if not (1 <= ans <= len(opts)):
            ans = 1  # default to first option rather than discarding
        ...
        out.append({"question": q, "options": opts, "answer": ans, "explanation": expl or "See solution above."})
        if len(out) >= 3:
            break
...
# ── 3. SPECIFIC QUESTION TARGETING - ALL questions from page, any Q number ──

def _pick_all_mcq_items_for_instruction(items: List[Dict[str, Any]], extra_instruction: str) -> List[Dict[str, Any]]:
    """
    FIXED: Returns ALL matching items for a given instruction, not just the first.
    If instruction says 'Q8' or 'question 8', returns that specific question.
    If no specific question is requested, returns first item.
    """
    pool = [dict(x) for x in (items or []) if str((x or {}).get("questions") or "").strip()]
    if not pool:
        return []
    ...
    # Check if user wants ALL questions
    all_keywords = re.compile(r"\b(all|সব|সকল|all questions|সব প্রশ্ন)\b", re.IGNORECASE)
    if all_keywords.search(ins):
        return pool

    # Check for specific question number
    qno = _normalize_question_no_token(ins)
    if qno:
        ...
        # Try positional (if qno=8, get 8th item)
        try:
            idx = int(qno) - 1
            if 0 <= idx < len(pool):
                return [pool[idx]]
...
# ── 4. GROUP REPLY WITHOUT QUOTE ──
# Monkey-patch safe_reply to suppress reply_to_message_id in groups

_orig_safe_reply = safe_reply

async def safe_reply(update: Update, text: str, **kwargs) -> Optional[Any]:
    """
    FIXED: In groups, send message WITHOUT quoting (no reply_to_message_id).
    In private chats, keep normal reply behavior.
    """
    try:
        if not update or not update.message:
            return await _orig_safe_reply(update, text, **kwargs)
        chat_type = str(getattr(update.effective_chat, "type", "") or "")
        if chat_type in ("group", "supergroup"):
            # Send without quoting in groups
            with contextlib.suppress(Exception):
...
# ── 5. TELEGRAM STREAMING (Live typing effect for AI responses) ──

async def _send_streaming_response(
    message,  # telegram Message object to reply to
    generate_fn,  # callable() -> str (blocking AI call)
    role: str = "USER",
    *,
    ...
    """
    Send AI response with live streaming effect using Telegram's editMessageText.
    Shows a typing indicator, then sends partial updates every ~0.8s.
    Falls back to single send if streaming fails.
    """
    bot = message.bot
    ...
    # Run the blocking AI call in executor
    loop = asyncio.get_running_loop()
    executor, sem = _pick_executor_and_sem(role)
    answer = None
    error_text = None
    ...
    try:
        ...
        error_text = "⏱ AI response timed out. Please try again."
    except Exception as e:
        error_text = f"❌ Error: {str(e)[:200]}"

    final_text = answer if answer else (error_text or "❌ No response received.")

    # Edit placeholder with final answer (streaming effect done via single edit)
    if placeholder_msg:
        try:
...
# ── 6. REPLY-TO-BOT-RESPONSE FLOW CONTINUITY ──
# When user replies to ANY bot message (not just OCR), continue the conversation

_KNOWN_BOT_RESPONSE_PATTERN = re.compile(
    r"(Gemini|Perplexity|DeepSeek|AI|✨|⚛|Solving|Answer|ব্যাখ্যা)",
    re.IGNORECASE
)

def _reply_is_to_bot_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Check if the user is replying to a bot AI response message."""
    msg = update.message
    if not msg or not msg.reply_to_message:
        return False
    replied = msg.reply_to_message
    # Check if replied message is from this bot
    bot_id = None
    with contextlib.suppress(Exception):
        bot_id = context.bot.id
    from_id = getattr(getattr(replied, "from_user", None), "id", None)
    if bot_id and from_id and from_id == bot_id:
        return True
    return False


def _extract_thread_context_from_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[str]:
    """Extract conversation context when user replies to a bot message."""
    msg = update.message
    if not msg or not msg.reply_to_message:
        return None
    replied = msg.reply_to_message
    prev_text = str(replied.text or replied.caption or "").strip()
    user_text = str(msg.text or "").strip()
    if not prev_text or not user_text:
        return None
    return (
        f"Previous bot response (context):\n{prev_text[:1500]}\n\n"
        f"User follow-up question:\n{user_text}"
    )


# Override the user reply handler to support reply-to-bot-answer flow
_prev_handle_user_reply_ocr = handle_user_reply_ocr_question

async def handle_user_reply_ocr_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    FIXED: Handle both OCR replies AND replies to bot's AI responses.
    Now properly continues conversation when user replies to any bot response.
    """
    ensure_user(update)
    if not update.message or not update.effective_user or not is_private_chat(update):
        return
    ...
    reply_has_ctx = _has_ocr_context(context, reply_msg)
    reply_is_media = _is_supported_ocr_media_message(reply_msg)
    is_bot_reply = _reply_is_to_bot_answer(update, context)

    # Handle reply-to-bot-answer: continue conversation in same thread
    if is_bot_reply and not reply_is_media and not reply_has_ctx:
        thread_context = _extract_thread_context_from_reply(update, context)
        if thread_context and solver_mode_on(uid):
            token = _make_token()
            store = _pending_store(context)
            store[token] = {
                "uid": uid,
            ...
            try:
                await update.message.reply_text(
                    ui_box_text("Continue Conversation", "Choose AI to continue this discussion.", emoji="💬"),
                    parse_mode=ParseMode.HTML,
                    disable_web_page_preview=True,
                    reply_markup=_solver_picker_kb(token),
                )
    ...
    # Original OCR reply flow
    if not reply_is_media and not reply_has_ctx:
        return

    # Delegate to original handler
    try:
        await _prev_handle_user_reply_ocr(update, context)
    except ApplicationHandlerStop:
        raise
    except Exception:
        pass
...
# ── 7. FIX: /qans with specific question number from full page ──

_prev_qans_cmd_final = cmd_qans

async def cmd_qans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    FIXED cmd_qans: Properly picks specific question from full page OCR.
    If user says '/qans 8', it picks question 8 from the page.
    If user says '/qans all', posts all questions.
    """
    if not update.message or not update.effective_user:
        return
    if not is_private_chat(update):
        await warn(update, "Private Only", "Use this command in private chat.")
        return
    uid = update.effective_user.id
    if not _can_use_staff_ocr(uid):
        await warn_unauthorized(update, "Only Owner/Admin OCR users can use this command.")
    ...
    reply_msg = update.message.reply_to_message
    if not reply_msg:
        await safe_reply(update, usage_box("qans", "[Q number or keyword]",
            "Reply to an image, PDF, text, or quiz, then use:\n"
            "/qans — solve first MCQ\n"
            "/qans 8 — solve question 8\n"
            "/qans all — solve all questions on page"))
        return
    ...
    # Check if user wants ALL questions
    all_re = re.compile(r"\b(all|সব|সকল)\b", re.IGNORECASE)
    want_all = bool(all_re.search(extra_instruction))
    ...
    try:
        # Handle poll reply
        if getattr(reply_msg, "poll", None):
            await _prev_qans_cmd_final(update, context)
            return

        # Get OCR context if not cached
        if not ocr_ctx and _is_supported_ocr_media_message(reply_msg):
            if not mistral_runtime_enabled() or not get_mistral_api_key():
                raise RuntimeError("Mistral OCR not ready. Use /mistral status first.")
            suffix = ".jpg"
            if reply_msg.document:
                name = str(reply_msg.document.file_name or "").lower()
                suffix = ".pdf" if name.endswith(".pdf") else ((os.path.splitext(name)[1].strip() or ".jpg")[:6] or ".jpg")
                tg_file = await reply_msg.document.get_file()
            else:
                tg_file = await reply_msg.photo[-1].get_file()
            with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as f:
                local_path = f.name
            await tg_file.download_to_drive(local_path)
            proc = await _processing_start(update.message, "OCR Scan", "Extracting all questions from page...")
            bundle = await _run_blocking(_role_of(uid), _extract_ocr_bundle_from_path, local_path, uid, timeout=300)
            ocr = bundle["ocr"]
            ocr_ctx = {
                "raw_markdown": str(ocr.get("raw_markdown") or ""),
        ...
        if want_all and ocr_ctx:
            all_items = list(ocr_ctx.get("items") or [])
            if not all_items:
                raise RuntimeError("No MCQ items found on this page.")

            await safe_reply(update, ui_box_html(
                "All Questions Found",
                f"Found <code>{h(str(len(all_items)))}</code> MCQ(s) on this page.\nPosting all now...",
                emoji="📋"
            ))
            for i, item in enumerate(all_items[:20]):  # max 20 to avoid flood
                options = [str(item.get(f"option{j}") or "").strip() for j in range(1, 6)
                           if str(item.get(f"option{j}") or "").strip()]
                q = str(item.get("questions") or "").strip()
                if not q or len(options) < 2:
                    continue
                official_ans = int(item.get("answer", 0) or 0)
                official_expl = str(item.get("explanation") or "").strip()
                token = _make_token()
                store = _pending_store(context)
                store[token] = {
                    "uid": uid,
                ...
                qno_label = _item_question_no(item) or str(i + 1)
                try:
                    await update.message.reply_text(
                        ui_box_html(f"Q{qno_label}", h(q[:300]), emoji="❓"),
                        parse_mode=ParseMode.HTML,
                        disable_web_page_preview=True,
                        reply_markup=_solver_picker_kb(token),
                    )
        ...
        # Single question mode: pick specific question
        if ocr_ctx:
            items = list(ocr_ctx.get("items") or [])
            picked_list = _pick_all_mcq_items_for_instruction(items, extra_instruction)
            picked = picked_list[0] if picked_list else None

            if picked:
                options = [str(picked.get(f"option{j}") or "").strip() for j in range(1, 6)
                           if str(picked.get(f"option{j}") or "").strip()]
                q = str(picked.get("questions") or "").strip()
                if q and len(options) >= 2:
                    ...
                    label = f"Q{qno}" if qno else "Question"
                    if extra_instruction and not qno:
                        label = "Matched Question"

                    await _processing_delete(proc)
                    proc = None
                    await update.message.reply_text(
                        ui_box_html(label, h(q[:300]) + (f"\n\n<i>Specify Q number for a different question. Use /qans all for all.</i>" if not extra_instruction else ""), emoji="❓"),
                        parse_mode=ParseMode.HTML,
                        disable_web_page_preview=True,
                        reply_markup=_solver_picker_kb(token),
                    )
            ...
            # Fallback: text-based solve from OCR
            source_text = str(ocr_ctx.get("clean_text") or ocr_ctx.get("raw_markdown") or "").strip()
            if extra_instruction:
                source_text = f"From this page, answer: {extra_instruction}\n\n{source_text}"
            token = _make_token()
            store = _pending_store(context)
            store[token] = {
                "uid": uid,
            ...
            await update.message.reply_text(
                ui_box_text("Choose AI Model", "Tap to get answer from OCR text.", emoji="🤖"),
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
                reply_markup=_solver_picker_kb(token),
            )
        ...
        # No OCR - use plain text from replied message
        await _prev_qans_cmd_final(update, context)
        ...
        await _processing_delete(proc)
        db_log("ERROR", "qans_fixed_failed", {"user_id": uid, "error": str(e)})
        await err(update, "Question Answer Failed", str(e)[:220])
    finally:
        if local_path:
            with contextlib.suppress(Exception):
...
# ── 8. FIXED: Solver callback — dedup lock + no double-answer + no stuck ──
# Key fixes:
# (a) Use a _SOLVING_LOCKS set to prevent double-processing when user taps rapidly
# (b) Remove token from store BEFORE processing to prevent repeat runs
# (c) Do NOT call _prev wrapper — directly implement full logic here
# (d) Always answer callback_query exactly once

_SOLVING_LOCKS: set = set()  # tracks (uid, token) pairs currently being solved

async def on_solver_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """FIXED v3: No double callback, no stuck, dedup-guarded solver."""
    if not update.callback_query:
        return
    q = update.callback_query
    ...
    if not isinstance(req, dict):
        with contextlib.suppress(Exception):
            await q.answer("⚠️ অনুরোধটি মেয়াদোত্তীর্ণ। আবার প্রশ্ন পাঠান।", show_alert=True)
        return

    uid = int(req.get("uid") or 0)
    if q.from_user and q.from_user.id != uid:
        with contextlib.suppress(Exception):
            await q.answer("এটি আপনার অনুরোধ নয়।", show_alert=True)
        return

    # ── DEDUP LOCK: prevent double-processing on rapid taps ──
    lock_key = (uid, token, model)
    if lock_key in _SOLVING_LOCKS:
        with contextlib.suppress(Exception):
            await q.answer("⏳ ইতিমধ্যে প্রক্রিয়া চলছে...", show_alert=False)
        return
    _SOLVING_LOCKS.add(lock_key)

    # ── Answer the callback ONCE immediately ──
    with contextlib.suppress(Exception):
        await q.answer("⏳ AI চিন্তা করছে...", show_alert=False)
    ...
    try:
        if kind == "poll" and payload.get("question"):
            ...
            raw_why_not = result.get("why_not", {}) or {}
            clean_why_not = {k: clean_latex(v) for k, v in raw_why_not.items()}
            model_name = {"G": "Gemini", "P": "Perplexity", "D": "DeepSeek"}.get(model, model)
            msg_html = _format_user_poll_solution(
                question=question,
                options=options,
                model_ans=int(result.get("answer", 0) or 0),
        ...
        db_log("ERROR", "solver_callback_v3_failed", {"user_id": uid, "model": model, "error": str(e)})
        with contextlib.suppress(Exception):
            await q.edit_message_text(
                ui_box_html("Solve Failed", h(str(e)[:180]), emoji="❌"),
                parse_mode=ParseMode.HTML,
            )
    finally:
        # ── Always release the dedup lock ──
        _SOLVING_LOCKS.discard(lock_key)


# ── Re-register: Final build_app override with all v3 fixes ──
_prev_build_app_final_fix = build_app

def build_app() -> Application:
    app = _prev_build_app_final_fix()

    private_filter = filters.ChatType.PRIVATE

    # ── IMPORTANT: Remove ALL existing solve: callback handlers from ALL groups ──
    # This prevents duplicate firing when user taps Gemini/Perplexity buttons.
    # We iterate all handler groups and remove any CallbackQueryHandler with solve: pattern.
    _solve_pattern = re.compile(r"^solve:[GPD]:[0-9a-f]{6,16}$")
    for group_id, handler_list in list(app.handlers.items()):
        to_remove = []
        for h_ in list(handler_list):
            if isinstance(h_, CallbackQueryHandler):
                p = getattr(h_, "pattern", None)
                p_str = p.pattern if hasattr(p, "pattern") else str(p or "")
                if "solve" in p_str and ("GPD" in p_str or "solve:" in p_str):
                    to_remove.append(h_)
        for h_ in to_remove:
            with contextlib.suppress(Exception):
                app.remove_handler(h_, group_id)

    # ── Add single, dedup-guarded solve handler at highest priority ──
    app.add_handler(
        CallbackQueryHandler(
            on_solver_callback,
            pattern=r"^solve:[GPD]:[0-9a-f]{6,16}$",
        ),
        group=-50,
    )

    # Log file download button callback
    with contextlib.suppress(Exception):
        app.add_handler(
            CallbackQueryHandler(
                _on_log_download_callback,
                pattern=r"^owner_dl_log:",
            ),
            group=-50,
        )

    # Reply OCR question handler
    with contextlib.suppress(Exception):
        app.add_handler(
            MessageHandler(
                private_filter & filters.TEXT & filters.REPLY & (~filters.COMMAND),
                handle_user_reply_ocr_question,
            ),
            group=-50,
        )

    # /logs and /ownerstats command
    with contextlib.suppress(Exception):
        app.add_handler(
            CommandHandler(
                ["logs", "ownerstats"],
    ...
    # Start midnight OCR reset task
    with contextlib.suppress(Exception):
        loop = asyncio.get_event_loop()
        if loop.is_running():
            loop.create_task(_ocr_midnight_reset_loop())
        else:
            app.post_init = _start_ocr_midnight_reset_task

    logger.info("[FINAL FIX v3 2026-04-12-C] All fixes applied: dedup solve callback (no double-answer/stuck), duplicate handler removal, robust score reply with fallback, OCR midnight reset, log button.")
    return app


logger.info("[FINAL FIX v3 2026-04-12-C] Patch loaded successfully. Fixes: (1) AI button no repeat/stuck, (2) score reply guaranteed, (3) no duplicate handlers.")

# ===== END FINAL COMPREHENSIVE FIX PATCH v3 =====


# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  PATCH v4 — 2026-04-12-D  (Professional User Image Reply + Daily Limit) ║
# ║  Fixes:                                                                  ║
# ║  1. Users: replying to an image in private triggers AI picker (like      ║
# ║     /qans does for owner/admin) — full OCR + solve flow.                 ║
# ║  2. Daily limit count shown in every AI response to users.               ║
# ║  3. Midnight reset (BST 00:00) notifies the owner when all limits reset.  ║
# ║  4. Gemini returning "অনুগ্রহ করে আপনার প্রশ্নটি পাঠান।" — fixed.      ║
# ║  5. "⏳ AI চিন্তা করছে…" spinner → professional English.                ║
# ║  6. Callback answer text → English.                                      ║
# ╚══════════════════════════════════════════════════════════════════════════╝

# ── helper: build a compact daily-limit footer for user responses ──
def _user_limit_footer_html(user_id: int) -> str:
    """Return an HTML footer line showing remaining daily OCR uses."""
    try:
        remaining = _remaining_user_ocr_quota(user_id)
        limit = get_mistral_user_daily_limit()
        used = limit - remaining
        bar_total = min(limit, 10)
        bar_filled = min(int(used * bar_total / max(limit, 1)), bar_total)
        bar = "█" * bar_filled + "░" * (bar_total - bar_filled)
        return (
            f"\n\n━━━━━━━━━━━━━━━━━━━━\n"
            f"📊 <b>Daily Usage</b>: <code>{bar}</code> "
            f"<b>{used}/{limit}</b>  ·  "
            f"<b>{remaining}</b> remaining today\n"
            f"<i>Resets at midnight (BD time)</i>"
        )
    except Exception:
        return ""


# ── FIX 1: User private image → AI picker (exactly like /qans for admin) ──

async def _handle_user_private_image_for_qans(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    When a regular USER sends (or replies to) an image in private chat,
    run the full OCR pipeline and immediately present the Gemini / Perplexity
    AI-picker buttons — identical to what /qans does for owner/admin.
    """
    ensure_user(update)
    if not update.message or not update.effective_user:
        return
    ...
    if is_banned(uid):
        return
    # Only for plain USERs (owner/admin already have their own richer flow)
    if get_role(uid) != ROLE_USER:
        return

    msg = update.message

    # Figure out which message actually carries the photo
    src_msg = msg
    if msg.reply_to_message and _is_supported_ocr_media_message(msg.reply_to_message):
        src_msg = msg.reply_to_message
    elif not _is_supported_ocr_media_message(msg):
        return

    # Guard: Mistral must be active
    if not mistral_runtime_enabled() or not get_mistral_api_key():
        return  # silently ignore if OCR infra not ready
    ...
    try:
        if src_msg.photo:
            tg_file = await src_msg.photo[-1].get_file()
            suffix = ".jpg"
        elif src_msg.document:
            name = str(src_msg.document.file_name or "").lower()
            suffix = ".pdf" if (name.endswith(".pdf") or str(getattr(src_msg.document, "mime_type", "") or "").lower() == "application/pdf") else (os.path.splitext(name)[1].strip() or ".jpg")[:6]
            tg_file = await src_msg.document.get_file()
        else:
            return
    except Exception:
        return
    ...
    try:
        ...
        proc = await _processing_start(
            msg,
            "🔍 Processing Image",
            "Extracting text and questions from your image. Please wait…",
        )

        bundle = await _run_blocking(
            _role_of(uid),
            _extract_ocr_bundle_from_path,
            local_path,
            uid,
            timeout=300,
        )
        ...
        # Cache OCR context so follow-up text replies work
        ocr_ctx = {
            "raw_markdown": str(ocr.get("raw_markdown") or ""),
            "clean_text": clean_text,
            "items": items,
        ...
        _remember_ocr_context(context, src_msg.message_id, ocr_ctx)

        # Consume 1 daily quota use
        _inc_user_ocr_usage(uid, 1)

        # Build solve payload — prefer MCQ if found, else raw text
        payload = None
        kind = "text"
        prompt_note = "Choose an AI model to solve the question from your image."

        picked = _pick_first_mcq_item(items)
        if picked:
            options = [
                str(picked.get(f"option{i}") or "").strip()
                for i in range(1, 6)
            ...
            if q and len(options) >= 2:
                ...
                kind = "poll"
                prompt_note = "Choose an AI model to solve the MCQ extracted from your image."

        if payload is None:
            if not clean_text:
                await _processing_delete(proc)
                proc = None
                limit_footer = _user_limit_footer_html(uid)
                await safe_reply(
                    update,
                    ui_box_html(
                        "No Text Found",
                        "Could not extract readable text or MCQs from this image."
                        + limit_footer,
                        emoji="⚠️",
                    ),
                )
                raise ApplicationHandlerStop
            payload = {"text": clean_text, "source_user_text": clean_text}
            kind = "text"
            prompt_note = "Choose an AI model to answer from the OCR text."
        ...
        await msg.reply_text(
            ui_box_html("📷 Image Processed", prompt_note + limit_footer, emoji="🤖"),
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True,
            reply_markup=_solver_picker_kb(token),
        )
        ...
        await _processing_delete(proc)
        db_log("ERROR", "user_image_qans_failed", {"user_id": uid, "error": str(e)})
        with contextlib.suppress(Exception):
            await safe_reply(update, ui_box_html("Processing Failed", h(str(e)[:200]), emoji="❌"))
    finally:
        if local_path:
            with contextlib.suppress(Exception):
                os.remove(local_path)
...
# ── FIX 2: Inject daily-limit footer into solver callback response for users ──

_prev_on_solver_callback_v4 = on_solver_callback

async def on_solver_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """v4: Adds daily-limit footer to text responses for regular users."""
    await _prev_on_solver_callback_v4(update, context)

    # After the parent finishes, check if the message now has content
    # and user is a regular USER — if so, we cannot easily append here
    # because the parent already edited. Instead, we patch the
    # callback answer text below (FIX 5) and let the footer appear
    # via the updated handle_user_reply_ocr_question flow.


# ── FIX 3: Enhanced midnight reset loop — notifies owner on each reset ──

_prev_ocr_midnight_reset_loop_v4 = _ocr_midnight_reset_loop

async def _ocr_midnight_reset_loop() -> None:
    """v4: Resets user OCR daily limits at midnight BST and notifies the owner."""
    _BD_TZ = dt.timezone(dt.timedelta(hours=6))
    _already_started = False

    while True:
        try:
            now_bd = dt.datetime.now(_BD_TZ)
            next_midnight = (now_bd + dt.timedelta(days=1)).replace(
                hour=0, minute=0, second=5, microsecond=0
            )
            wait_seconds = (next_midnight - now_bd).total_seconds()
            ...
            # Clean old usage records (keep last 3 days)
            new_day = _today_bd_key()
            try:
                cutoff = (dt.datetime.now(_BD_TZ) - dt.timedelta(days=3)).strftime("%Y-%m-%d")
                conn = db_connect()
                cur = conn.cursor()
                cur.execute("DELETE FROM user_ocr_daily_usage WHERE day_key < ?", (cutoff,))
                deleted = int(cur.rowcount or 0)
                conn.commit()
                conn.close()
                logger.info("[OCR RESET v4] Daily limits refreshed. New day: %s, Deleted old rows: %d", new_day, deleted)
            except Exception as db_err:
                logger.warning("[OCR RESET v4] DB cleanup error: %s", db_err)
                deleted = 0
            ...
            try:
                ...
                limit = get_mistral_user_daily_limit()
                notify_text = (
                    f"🔄 <b>Daily Limit Reset</b>\n\n"
                    f"✅ All user daily OCR/AI limits have been refreshed for <b>{new_day}</b> (BD time).\n"
                    f"📊 Each user now has <b>{limit}</b> use(s) available today.\n"
                    f"🗑️ Cleaned <b>{deleted}</b> old usage record(s) from the database."
                )
                ...
                logger.warning("[OCR RESET v4] Owner notification failed: %s", notify_err)

        except asyncio.CancelledError:
            break
        except Exception as loop_err:
            logger.warning("[OCR RESET v4] Loop error: %s", loop_err)
            await asyncio.sleep(60)


# ── FIX 4: gemini_solve_text — suppress empty/prompt-only replies ──

_prev_gemini_solve_text_v4 = gemini_solve_text

def gemini_solve_text(problem_text: str) -> str:
    """v4: Falls back to Perplexity when Gemini returns an empty or prompt-mirroring reply."""
    _EMPTY_PHRASES = {
        "অনুগ্রহ করে আপনার প্রশ্নটি পাঠান",
    ...
    try:
        result = _prev_gemini_solve_text_v4(problem_text)
        result_stripped = (result or "").strip()
        result_lower = result_stripped.lower()
        # Reject if empty or matches known empty-response phrases
        if not result_stripped:
            raise RuntimeError("Gemini returned empty response.")
        for phrase in _EMPTY_PHRASES:
            if phrase in result_lower:
                raise RuntimeError(f"Gemini returned a prompt-mirror response: {result_stripped[:80]}")
        return result
    except RateLimitError:
        raise
    except Exception as gemini_err:
        logger.warning("[gemini_solve_text v4] Falling back to Perplexity: %s", gemini_err)
        # Perplexity fallback
        if USE_PERPLEXITY_FALLBACK:
            try:
                result_p = perplexity_solve_text(problem_text)
                if result_p and result_p.strip():
                    return result_p
            except Exception as perp_err:
                logger.warning("[gemini_solve_text v4] Perplexity also failed: %s", perp_err)
        raise gemini_err


# ── FIX 4b: gemini_solve_mcq_json — same empty-reply guard ──

_prev_gemini_solve_mcq_json_v4 = gemini_solve_mcq_json

def gemini_solve_mcq_json(question: str, options: List[str]) -> Dict[str, Any]:
    """v4: Falls back to Perplexity when Gemini MCQ solve returns empty explanation."""
    _EMPTY_EXPL_PHRASES = {
        "অনুগ্রহ করে",
        "please send",
        "please provide",
    ...
    try:
        result = _prev_gemini_solve_mcq_json_v4(question, options)
        expl_lower = str(result.get("explanation") or "").strip().lower()
        if not expl_lower or any(phrase in expl_lower for phrase in _EMPTY_EXPL_PHRASES):
            raise RuntimeError("Gemini MCQ returned empty/prompt-mirror explanation.")
        return result
    except RateLimitError:
        raise
    except Exception as gemini_err:
        logger.warning("[gemini_solve_mcq_json v4] Falling back to Perplexity: %s", gemini_err)
        if USE_PERPLEXITY_FALLBACK:
            try:
                result_p = perplexity_solve_mcq_json(question, options)
                if isinstance(result_p, dict) and result_p.get("explanation"):
                    return result_p
            except Exception:
                pass
        raise gemini_err
...
# ── FIX 5: Professional English spinner & callback answer text ──

# Patch all "⏳ AI চিন্তা করছে..." occurrences at the callback answer level
# by overriding the final on_solver_callback's answer text.

_prev_on_solver_callback_final = on_solver_callback

async def on_solver_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """v4-final: Uses professional English spinner text."""
    if not update.callback_query:
        return
    q = update.callback_query
    ...
    if not isinstance(req, dict):
        with contextlib.suppress(Exception):
            await q.answer("⚠️ This request has expired. Please send your question again.", show_alert=True)
        return
    ...
    model_label = {"G": "Gemini", "P": "Perplexity", "D": "DeepSeek"}.get(model, "AI")
    with contextlib.suppress(Exception):
        await q.answer(f"⏳ {model_label} is thinking…", show_alert=False)

    with contextlib.suppress(Exception):
        await q.edit_message_text(
            f"⏳ <i>{model_label} is analysing your question…</i>",
            parse_mode=ParseMode.HTML,
            disable_web_page_preview=True,
        )
    if q.message:
    ...
    try:
        if kind == "poll" and payload.get("question"):
            ...
            msg_html = _format_user_poll_solution(
                ...
                model_ans=int(result.get("answer", 0) or 0),
                official_ans=int(payload.get("official_ans", 0) or 0),
                model_expl=f"[{model_label}]\n{clean_expl}".strip(),
                official_expl=str(payload.get("official_expl", "")).strip(),
                why_not=clean_why_not,
                conf=int(result.get("confidence", 0) or 0),
            )
            # Append daily-limit footer for regular users
            if get_role(uid) == ROLE_USER:
                msg_html += _user_limit_footer_html(uid)
            kb = _verify_kb(token, model, "poll")
        ...
        db_log("ERROR", "solver_callback_v4_failed", {"user_id": uid, "model": model, "error": str(e)})
        with contextlib.suppress(Exception):
            await q.edit_message_text(
                ui_box_html("AI Error", h(str(e)[:200]), emoji="❌"),
                parse_mode=ParseMode.HTML,
...
# ── FIX 6: User OCR-reply also shows daily limit footer ──

_prev_handle_user_reply_ocr_v4 = handle_user_reply_ocr_question

async def handle_user_reply_ocr_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """v4: Wraps the existing handler; injects daily-limit footer after response."""
    # We call the existing handler; footer is already injected in the
    # handle_user_reply_ocr_question body at line 17167 via usage_line.
    # Here we just ensure the 'from_user_image' path (new handler above)
    # is correctly skipped if this message is a plain text reply to OCR.
    await _prev_handle_user_reply_ocr_v4(update, context)


# ── FIX 7: Register new USER image handler + patch midnight reset ──
...
def build_app() -> Application:
    ...
    # ── Remove all existing solve callback handlers (again, to replace with v4) ──
    for group_id, handler_list in list(app.handlers.items()):
        to_remove = []
        for h_ in list(handler_list):
            if isinstance(h_, CallbackQueryHandler):
    ...
    # ── NEW: User private photo/image → AI picker ──
    # Priority -60 so it fires before the generic text reply handler
    app.add_handler(
        MessageHandler(
            private_filter & (filters.PHOTO | (filters.Document.IMAGE | filters.Document.PDF)),
            _handle_user_private_image_for_qans,
        ),
        group=-60,
    )

    # ── Also handle text replies to a cached image (already covered by
    #    existing handle_user_reply_ocr_question — no change needed) ──

    # ── Patch midnight reset loop ──
    async def _post_init_v4(application) -> None:
        with contextlib.suppress(Exception):
            asyncio.create_task(_ocr_midnight_reset_loop())
            logger.info("[PATCH v4] OCR midnight reset + owner-notify loop started.")

    with contextlib.suppress(Exception):
        app.post_init = _post_init_v4

    logger.info(
        "[PATCH v4 2026-04-12-D] Applied: "
        "(1) user-image→AI-picker, "
        "(2) daily-limit footer in responses, "
        "(3) midnight reset + owner notification, "
        "(4) Gemini empty-reply fallback to Perplexity, "
        "(5) professional English spinner."
    )
    return app


logger.info("[PATCH v4 2026-04-12-D] Module loaded successfully.")
...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  PROBAHO PROFESSIONAL PATCH v4  —  2026-04-12-D                            ║
# ║  Author: Anthropic Claude (automated patch)                                 ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
# ║  CHANGES:                                                                   ║
# ║  1. User inbox: replying to any image triggers OCR+AI (like /qans)         ║
# ║  2. Daily limit shown in every AI response for users                        ║
# ║  3. Gemini "অনুগ্রহ করে…" empty-response bug fixed for groups              ║
# ║  4. Midnight reset sends notification to Owner                              ║
# ║  5. AI thinking status → professional English                               ║
# ║  6. All user-facing messages polished to professional English               ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

import asyncio as _asyncio_v4
import contextlib as _contextlib_v4

# ─────────────────────────────────────────────────────────────────────────────
# PATCH 1 — User inbox photo reply works exactly like /qans (OCR + AI picker)
# ─────────────────────────────────────────────────────────────────────────────

async def _handle_user_private_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    When a regular USER sends or replies-to a photo in private chat,
    treat it exactly like /qans — run Mistral OCR and present AI picker.
    Respects daily OCR limit and shows remaining quota in the response.
    """
    ensure_user(update)
    if not update.message or not update.effective_user:
        return
    ...
    role = get_role(uid)
    # Admins / owners go through the existing handle_image pipeline
    if role in (ROLE_ADMIN, ROLE_OWNER):
        return  # Let existing handler deal with it

    # Only regular users are handled here
    if role != ROLE_USER:
        return

    # Determine the image message (could be direct send or a reply)
    if update.message.photo:
        photo_msg = update.message
    elif update.message.reply_to_message and _is_supported_ocr_media_message(update.message.reply_to_message):
    ...
    if not mistral_runtime_enabled():
        await safe_reply(
            update,
            "⚠️ <b>OCR Unavailable</b>\nImage reading is currently disabled by the owner.",
        )
        return
    if not get_mistral_api_key():
        await safe_reply(
            update,
            "⚠️ <b>OCR Not Configured</b>\nThe bot owner hasn't set up an OCR key yet.",
        )
        return

    # Daily limit check — use cached OCR if already processed
    cached = _get_ocr_context(context, photo_msg.message_id)
    if not cached:
        remaining = _remaining_user_ocr_quota(uid)
        limit = get_mistral_user_daily_limit()
        if remaining <= 0:
            await safe_reply(
                ...
                (
                    ...
                    f"You have used all <b>{limit}</b> of your daily image scans.\n"
                    "Your limit resets automatically at <b>midnight (Bangladesh time)</b>.\n"
    ...
    try:
        ...
        if not ocr_ctx:
            # Download image
            if photo_msg.document:
                name = str(photo_msg.document.file_name or "").lower()
                suffix = ".pdf" if (name.endswith(".pdf") or str(getattr(photo_msg.document, "mime_type", "") or "").lower() == "application/pdf") else ((os.path.splitext(name)[1].strip() or ".jpg")[:6] or ".jpg")
                tg_file = await photo_msg.document.get_file()
            else:
            ...
            proc = await _processing_start(
                update.message,
                "📷 Scanning Image",
                "⏳ Reading your image — this may take a few seconds...",
            )
            bundle = await _run_blocking(
            ...
            _remember_ocr_context(context, photo_msg.message_id, ocr_ctx)
            _inc_user_ocr_usage(uid, 1)
        ...
        limit = get_mistral_user_daily_limit()
        quota_note = f"\n\n<i>📊 Daily limit: <b>{remaining_after}/{limit}</b> scans remaining today.</i>"

        if not items:
            # No MCQ items — show raw OCR text with AI solve option
            source_text = str(ocr_ctx.get("clean_text") or ocr_ctx.get("raw_markdown") or "").strip()
            if not source_text:
                await safe_reply(
                    update,
                    "⚠️ <b>No Text Detected</b>\nThe image appears to be blank or unreadable." + quota_note,
                )
                return
            token = _make_token()
            store = _pending_store(context)
            ...
            await update.message.reply_text(
                (
                    "📄 <b>Image Scanned</b>\n"
                    "Text extracted successfully. Choose an AI to process it:" + quota_note
                ),
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
                reply_markup=_solver_picker_kb(token),
        ...
        if len(items) == 1:
            item = items[0]
            options = [str(item.get(f"option{j}") or "").strip() for j in range(1, 6) if str(item.get(f"option{j}") or "").strip()]
            q = str(item.get("questions") or "").strip()
            if q and len(options) >= 2:
                official_ans = int(item.get("answer", 0) or 0)
                official_expl = str(item.get("explanation") or "").strip()
                ...
                await update.message.reply_text(
                    (
                        ui_box_html("📚 Question Detected", h(q[:300]), emoji="❓")
                        + "\n\n<i>Select an AI model to solve this question:</i>"
                        + quota_note
                    ),
                    parse_mode=ParseMode.HTML,
                    disable_web_page_preview=True,
        ...
        await safe_reply(
            ...
            (
                ui_box_html(
                    "📋 Multiple Questions Found",
                    f"Found <b>{len(items)}</b> MCQ(s) on this page.\n"
                    "Reply to this image with a question number (e.g. <code>1</code>, <code>Q3</code>) "
                    "to solve a specific one, or reply with <code>all</code> to solve all." + quota_note,
                    emoji="🔍",
                )
            ),
        )
        # Store OCR context bound to this message for follow-up replies
        # (already stored via _remember_ocr_context above)

    except Exception as e:
        await _processing_delete(proc)
        db_log("ERROR", "user_private_photo_ocr_failed", {"user_id": uid, "error": str(e)})
        await safe_reply(
            update,
            f"❌ <b>Scan Failed</b>\nCould not process the image: <code>{h(str(e)[:200])}</code>\n"
...
# ─────────────────────────────────────────────────────────────────────────────
# PATCH 2 — User photo reply (text reply to a photo) triggers OCR follow-up
# ─────────────────────────────────────────────────────────────────────────────
...
async def handle_user_reply_ocr_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    v4 FIXED: Handles text reply to a photo message for regular users.
    If user replies to a photo with a question/number, run OCR and answer.
    Also delegates to previous handler for all existing flows.
    """
    ensure_user(update)
    if not update.message or not update.effective_user or not is_private_chat(update):
        return
    ...
    # NEW: If user replied to a PHOTO with a text (question or number), treat as /qans
    if _is_supported_ocr_media_message(reply_msg):
        if not mistral_runtime_enabled() or not get_mistral_api_key():
            await safe_reply(update, "⚠️ <b>OCR Unavailable</b>\nImage scanning is not configured.")
            return
        ...
        if not cached:
            ...
            if remaining <= 0:
                await safe_reply(
                    ...
                    (
                        "🚫 <b>Daily Limit Reached</b>\n"
                        f"You've used all <b>{limit}</b> daily image scans.\n"
                        "Your limit resets at <b>midnight (Bangladesh time)</b>. 🌙"
                    ),
                )
                from telegram.ext import ApplicationHandlerStop
                raise ApplicationHandlerStop
        ...
        try:
            ...
            if not ocr_ctx:
                ...
                await tg_file.download_to_drive(local_path)
                proc = await _processing_start(
                    update.message, "📷 Scanning Image", "⏳ Running OCR on the image..."
                )
                bundle = await _run_blocking(
                    _role_of(uid), _extract_ocr_bundle_from_path, local_path, uid, timeout=300
                )
            ...
            remaining_after = _remaining_user_ocr_quota(uid)
            limit = get_mistral_user_daily_limit()
            quota_note = f"\n\n<i>📊 <b>{remaining_after}/{limit}</b> scans remaining today.</i>"
            ...
            if want_all and items:
                await safe_reply(
                    update,
                    ui_box_html(
                        "All Questions",
                        f"Found <code>{len(items)}</code> MCQ(s). Posting all now..." + quota_note,
                        emoji="📋",
                    ),
                )
            ...
            # Specific question pick
            if items:
                picked_list = _pick_all_mcq_items_for_instruction(items, extra_instruction)
                picked = picked_list[0] if picked_list else None
                if picked:
                    options = [str(picked.get(f"option{j}") or "").strip() for j in range(1, 6) if str(picked.get(f"option{j}") or "").strip()]
                    q = str(picked.get("questions") or "").strip()
                    if q and len(options) >= 2:
                        token = _make_token()
                        store = _pending_store(context)
                        ...
                        await update.message.reply_text(
                            (
                                ui_box_html(label, h(q[:300]), emoji="❓")
                                + "\n\n<i>Choose an AI to solve:</i>"
                                + quota_note
                            ),
                            parse_mode=ParseMode.HTML,
                            disable_web_page_preview=True,
            ...
            source_text = str(ocr_ctx.get("clean_text") or ocr_ctx.get("raw_markdown") or "").strip()
            if extra_instruction:
                source_text = f"Answer this request from the image: {extra_instruction}\n\n{source_text}"
            token = _make_token()
            store = _pending_store(context)
            store[token] = {
                "uid": uid,
            ...
            await update.message.reply_text(
                (
                    ui_box_text("🤖 Choose AI Model", "Tap a model to get your answer.", emoji="🧠")
                    + quota_note
                ),
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
            ...
            await _processing_delete(proc)
            db_log("ERROR", "user_reply_photo_ocr_v4_failed", {"user_id": uid, "error": str(e)})
            await safe_reply(
                update,
    ...
    # Delegate all other cases to previous handler
    try:
        await _prev_handle_user_reply_ocr_v4(update, context)
    except ApplicationHandlerStop:
        raise
...
# ─────────────────────────────────────────────────────────────────────────────
# PATCH 3 — Daily limit shown in every AI response sent to users
# ─────────────────────────────────────────────────────────────────────────────

def _user_quota_footer_html(uid: int) -> str:
    """Return a compact daily-limit footer line for user responses."""
    try:
        role = get_role(uid)
        if role in (ROLE_ADMIN, ROLE_OWNER):
            return ""
        remaining = _remaining_user_ocr_quota(uid)
        limit = get_mistral_user_daily_limit()
        bar_filled = int((remaining / max(limit, 1)) * 5)
        bar = "█" * bar_filled + "░" * (5 - bar_filled)
        return f"\n\n<i>📊 Daily scans: <b>{remaining}/{limit}</b>  [{bar}]  — Resets at midnight (BD time)</i>"
    except Exception:
        return ""


# ─────────────────────────────────────────────────────────────────────────────
# PATCH 4 — Fix "অনুগ্রহ করে আপনার প্রশ্নটি পাঠান।" in Gemini group responses
# Root cause: The AI system prompt instructs Gemini to return that text for
# off-topic/greeting messages. When Gemini does so, the bot should NOT send
# that text to the user — instead, silently drop or use Perplexity fallback.
# ─────────────────────────────────────────────────────────────────────────────

_GEMINI_EMPTY_RESPONSE_PATTERNS = re.compile(
    r"(?i)(অনুগ্রহ\s*করে\s*আপনার\s*প্রশ্ন|please\s*send\s*your\s*question|আপনার\s*প্রশ্নটি\s*পাঠান|send\s*your\s*question)",
    re.UNICODE,
)


def _is_gemini_empty_response(text: str) -> bool:
    """Returns True if Gemini responded with the 'please ask a question' deflection."""
    t = str(text or "").strip()
    if not t:
        return True
    if len(t) < 120 and _GEMINI_EMPTY_RESPONSE_PATTERNS.search(t):
        return True
    return False
...
async def on_solver_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    v4 FIXED: After Gemini answers, if the response is the 'please ask a question'
    deflection message, automatically retry with Perplexity instead of
    sending that useless message to the user.
    """
    if not update.callback_query:
        return
    q = update.callback_query
    ...
    # Only intercept Gemini (G) responses
    if model != "G":
        await _prev_on_solver_callback_v4(update, context)
        return
    ...
    if lock_key in _SOLVING_LOCKS:
        with contextlib.suppress(Exception):
            await q.answer("⏳ Already processing…", show_alert=False)
        return
    _SOLVING_LOCKS.add(lock_key)

    with contextlib.suppress(Exception):
        await q.answer("⏳ Processing with Gemini…", show_alert=False)
    with contextlib.suppress(Exception):
        await q.edit_message_text(
    ...
    try:
        ...
        if kind == "poll" and payload.get("question"):
            ...
            result = await _run_blocking(_role_of(uid), gemini_solve_mcq_json, question, options)
            raw_expl = str(result.get("explanation", "") or "")
            if _is_gemini_empty_response(raw_expl):
                # Retry with Perplexity silently
                result = await _run_blocking(_role_of(uid), perplexity_solve_mcq_json, question, options)
                model_used_name = "Perplexity (auto-retry)"
            else:
                model_used_name = "Gemini"
            clean_expl = clean_latex(str(result.get("explanation", "") or ""))
            clean_why_not = {k: clean_latex(v) for k, v in (result.get("why_not") or {}).items()}
            msg_html = _format_user_poll_solution(
                question=question,
                options=options,
                model_ans=int(result.get("answer", 0) or 0),
                official_ans=int(payload.get("official_ans", 0) or 0),
                model_expl=f"[{model_used_name}]\n{clean_expl}".strip(),
                official_expl=str(payload.get("official_expl", "")).strip(),
                why_not=clean_why_not,
                conf=int(result.get("confidence", 0) or 0),
            )
            kb = _verify_kb(token, "G" if "Gemini" in model_used_name else "P", "poll")
        else:
            answer = await _run_blocking(_role_of(uid), gemini_solve_text, problem_text)
            if _is_gemini_empty_response(answer):
                ...
                model_used_name = "Gemini"
            msg_html = h(answer)
            kb = _verify_kb(token, "G" if "Gemini" in model_used_name else "P", "text")

        # Append quota footer for regular users
        quota_footer = _user_quota_footer_html(uid)
        ...
        if q.message and getattr(q.message.chat, "type", "") in ("group", "supergroup"):
            asyncio.create_task(
                _auto_delete_after(context.bot, q.message.chat_id, [q.message.message_id], 300)
            )

    except Exception as e:
        db_log("ERROR", "solver_callback_v4_gemini_fix_failed", {"user_id": uid, "error": str(e)})
        with contextlib.suppress(Exception):
            await q.edit_message_text(
                ui_box_html("AI Error", h(str(e)[:200]), emoji="❌"),
                parse_mode=ParseMode.HTML,
...
# ─────────────────────────────────────────────────────────────────────────────
# PATCH 5 — Midnight reset sends owner notification
# ─────────────────────────────────────────────────────────────────────────────

_prev_ocr_midnight_reset_loop = _ocr_midnight_reset_loop


async def _ocr_midnight_reset_loop() -> None:
    """v4: Midnight reset with owner notification."""
    _BD_TZ_v4 = dt.timezone(dt.timedelta(hours=6))
    _bot_ref = None  # Will be set lazily

    while True:
        try:
            now_bd = dt.datetime.now(_BD_TZ_v4)
            next_midnight = (now_bd + dt.timedelta(days=1)).replace(
                hour=0, minute=0, second=5, microsecond=0
            )
            wait_seconds = (next_midnight - now_bd).total_seconds()
            ...
            # Get user count before cleaning
            total_users_reset = 0
            try:
                conn = db_connect()
                cur = conn.cursor()
                today_key = _today_bd_key()
                cur.execute(
                    "SELECT COUNT(DISTINCT user_id) as c FROM user_ocr_daily_usage WHERE day_key = ?",
                    (today_key,),
                )
                row = cur.fetchone()
                total_users_reset = int(row["c"] or 0) if row else 0

                cutoff_date = (dt.datetime.now(_BD_TZ_v4) - dt.timedelta(days=3)).strftime("%Y-%m-%d")
                cur.execute("DELETE FROM user_ocr_daily_usage WHERE day_key < ?", (cutoff_date,))
                conn.commit()
                conn.close()
                logger.info("[OCR RESET v4] Daily limits reset. New day: %s", _today_bd_key())
            except Exception as e:
                logger.warning("[OCR RESET v4] Cleanup error: %s", e)

            # Notify owner
            try:
                # Lazy-get the bot application
                import telegram as _tg_v4
                from telegram import Bot as _Bot_v4
                bot_token_local = BOT_TOKEN
                if bot_token_local:
                    _tmp_bot = _Bot_v4(token=bot_token_local)
                    new_day_bd = dt.datetime.now(_BD_TZ_v4).strftime("%Y-%m-%d")
                    limit_val = get_mistral_user_daily_limit()
                    notif_text = (
                        "🔄 <b>Daily Limit Reset — Midnight</b>\n\n"
                        f"🕛 Time: <b>Midnight (Bangladesh)</b>\n"
                        f"📅 New day: <b>{new_day_bd}</b>\n"
                        f"👥 Users whose limits were reset: <b>{total_users_reset}</b>\n"
                        f"🎚 Daily scan limit per user: <b>{limit_val}</b>\n\n"
                        "All users can now use their full daily quota again. ✅"
                    )
                    ...
                    await _tmp_bot.close()
            except Exception as e:
                logger.warning("[OCR RESET v4] Owner notification error: %s", e)

        except asyncio.CancelledError:
            break
        except Exception as e:
            logger.warning("[OCR RESET v4] Loop error: %s", e)
            await asyncio.sleep(60)


# ─────────────────────────────────────────────────────────────────────────────
# PATCH 6 — Professional English for all AI status/thinking messages
# ─────────────────────────────────────────────────────────────────────────────

def _pro_thinking_html(model_name: str = "AI") -> str:
    return f"⏳ <i>{model_name} is analyzing your request…</i>"


# Patch the callback answer text used during solving
# (The "⏳ AI চিন্তা করছে..." message in the inline callback)
# This is already done inside on_solver_callback above.

# Also patch the limit warning for users to be professional English
async def _send_ocr_limit_warning(update: Update, daily_limit: int) -> None:
    """v4: Professional English OCR limit warning."""
    msg = (
        "🚫 <b>Daily Scan Limit Reached</b>\n\n"
        f"You have used all <b>{daily_limit}</b> of your daily image scans.\n"
        "Your quota resets automatically at <b>midnight (Bangladesh Standard Time)</b>.\n\n"
...
# ─────────────────────────────────────────────────────────────────────────────
# PATCH 7 — Register new handlers and override the build_app
# ─────────────────────────────────────────────────────────────────────────────
...
def build_app() -> Application:
    ...
    private_filter = filters.ChatType.PRIVATE
    user_role_filter = filters.ChatType.PRIVATE  # Additional role check is inside handler

    # ── Remove ALL existing solve: callback handlers so v4 takes priority ──
    _solve_pattern_v4 = re.compile(r"^solve:[GPD]:[0-9a-f]{6,16}$")
    for group_id, handler_list in list(app.handlers.items()):
        to_remove = []
        for h_ in list(handler_list):
            if isinstance(h_, CallbackQueryHandler):
    ...
    # ── v4 solve callback (Gemini empty-response fix + quota footer) ──
    app.add_handler(
        CallbackQueryHandler(
            on_solver_callback,
            pattern=r"^solve:[GPD]:[0-9a-f]{6,16}$",
    ...
    # ── User private photo handler (direct image send → OCR + AI) ──
    app.add_handler(
        MessageHandler(
            private_filter & filters.PHOTO & (~filters.COMMAND),
            _handle_user_private_photo,
        ),
        group=-100,
    )

    # ── User text reply to photo → OCR follow-up (override previous) ──
    # Remove old user-reply-ocr handlers
    for group_id, handler_list in list(app.handlers.items()):
        to_remove = []
        for h_ in list(handler_list):
            if isinstance(h_, MessageHandler):
                fn = getattr(h_, "callback", None)
                fn_name = getattr(fn, "__name__", "") if fn else ""
                if "handle_user_reply_ocr" in fn_name:
                    to_remove.append(h_)
        for h_ in to_remove:
            with contextlib.suppress(Exception):
                app.remove_handler(h_, group_id)
    ...
    # ── Override midnight reset loop with v4 (owner notification) ──
    with contextlib.suppress(Exception):
        loop = asyncio.get_event_loop()
        if loop.is_running():
            loop.create_task(_ocr_midnight_reset_loop())
    ...
    logger.info(
        "[PATCH v4 2026-04-12-D] Applied: "
        "(1) User inbox photo→OCR+AI, "
        "(2) Quota footer in responses, "
        "(3) Gemini empty-response auto-retry with Perplexity, "
        "(4) Midnight reset owner notification, "
        "(5) Professional English messages."
    )
    return app


logger.info("[PATCH v4 2026-04-12-D] Loaded successfully.")

# ===== END PROBAHO PROFESSIONAL PATCH v4 =====
