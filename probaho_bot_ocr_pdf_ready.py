...
# ===== OCR IMAGE + PDF QUIZ EXTRACTION PATCH (2026-03-28) =====
# Goal:
# - Keep existing bot behavior intact.
# - Add FREE local OCR-based extraction from images and PDFs.
# - Fall back to original Gemini image flow only when OCR does not find MCQs and API key exists.
...
OCR_LANGS = os.getenv('OCR_LANGS', 'eng+ben')
OCR_TESSERACT_CMD = os.getenv('TESSERACT_CMD', '').strip()
OCR_PAGE_LIMIT = max(1, int(os.getenv('OCR_PAGE_LIMIT', '35') or '35'))
OCR_MIN_QUESTION_LEN = max(8, int(os.getenv('OCR_MIN_QUESTION_LEN', '8') or '8'))
OCR_DEBUG = str(os.getenv('OCR_DEBUG', '0')).strip().lower() in {'1', 'true', 'yes', 'on'}

if pytesseract and OCR_TESSERACT_CMD:
    with contextlib.suppress(Exception):
        pytesseract.pytesseract.tesseract_cmd = OCR_TESSERACT_CMD

_BN_DIGIT_MAP = str.maketrans('০১২৩৪৫৬৭৮৯', '0123456789')
_BN_OPTION_MAP = {'ক': 'A', 'খ': 'B', 'গ': 'C', 'ঘ': 'D', 'ঙ': 'E'}
...
def _ocr_preprocess_image(img_path: str):
    if Image is None:
        raise RuntimeError('Pillow is not installed. Please install requirements.txt')
...
def _ocr_image_text(img_path: str) -> str:
    if not _ocr_engine_ready():
        raise RuntimeError('OCR dependencies missing. Install Pillow + pytesseract and system Tesseract OCR.')
    img = _ocr_preprocess_image(img_path)
    cfg = '--oem 3 --psm 6 preserve_interword_spaces=1'
    text = pytesseract.image_to_string(img, lang=OCR_LANGS, config=cfg)
    return str(text or '').strip()


def _pdf_text_or_ocr(pdf_path: str) -> str:
    if fitz is None:
        raise RuntimeError('PyMuPDF is not installed. Please install requirements.txt')
    texts = []
    doc = fitz.open(pdf_path)
    try:
        total = min(len(doc), OCR_PAGE_LIMIT)
        for i in range(total):
            page = doc.load_page(i)
            raw = str(page.get_text('text') or '').strip()
            ...
            try:
                ocr_text = _ocr_image_text(tmp_img.name)
...
def _is_question_start(line: str) -> bool:
    ...
    patterns = [
        r'^\d{1,3}[\).:-]\s+\S+',
        r'^\(?\d{1,3}\)\s+\S+',
        r'^(?:প্রশ্ন|question)\s*\d{0,3}\s*[:.-]?\s+\S+',
    ]
    return any(re.match(p, line, flags=re.I) for p in patterns)


def _option_match(line: str):
    line = str(line or '').strip()
    if not line:
        return None
    # English labels
    ...
    # Numeric labels 1-5 -> A-E
    m = re.match(r'^([1-5])[\).:-]\s*(.+)$', line)
    if m:
        return chr(ord('A') + int(m.group(1)) - 1), m.group(2).strip()
    # Bengali labels ক-ঙ
    m = re.match(r'^([কখগঘঙ])[\).:-]\s*(.+)$', line)
    if m:
        return _BN_OPTION_MAP.get(m.group(1), 'A'), m.group(2).strip()
    return None


def _answer_index_from_text(text: str) -> int:
    s = _normalize_ocr_text(text)
    m = re.search(r'(?:answer|ans|correct answer|উত্তর|সঠিক উত্তর|সমাধান)\s*[:\-]?\s*([A-Ea-e1-5কখগঘঙ])', s, flags=re.I)
    if not m:
        return 0
    token = m.group(1).strip()
    if token.isdigit():
        return int(token)
    if token in _BN_OPTION_MAP:
        token = _BN_OPTION_MAP[token]
    token = token.upper()
    if token in 'ABCDE':
        return ord(token) - 64
    return 0
...
def _clean_question_prefix(line: str) -> str:
    s = str(line or '').strip()
    s = re.sub(r'^\(?\d{1,3}\)?[\).:-]\s*', '', s)
    s = re.sub(r'^(?:প্রশ্ন|question)\s*\d{0,3}\s*[:.-]?\s*', '', s, flags=re.I)
    return s.strip()


def _parse_mcq_block(block_lines):
    lines = [str(x).strip() for x in (block_lines or []) if str(x).strip()]
    if not lines:
        return None
    q_lines = []
    opts = []
    ...
    for idx, line in enumerate(lines):
        ans_idx = _answer_index_from_text(line)
        if ans_idx and not answer_idx:
            answer_idx = ans_idx
            continue
        if re.match(r'^(?:explanation|ব্যাখ্যা|explain)\s*[:\-]?', line, flags=re.I):
            in_expl = True
            expl_lines.append(re.sub(r'^(?:explanation|ব্যাখ্যা|explain)\s*[:\-]?\s*', '', line, flags=re.I).strip())
            continue
        if in_expl:
            expl_lines.append(line)
            continue
        om = _option_match(line)
        if om:
            label, text = om
            opts.append((label, text))
            last_opt_idx = idx
            continue
        if opts and idx > last_opt_idx >= 0:
            # continuation of the previous option until answer/explanation/next question
            label, prev = opts[-1]
            opts[-1] = (label, (prev + ' ' + line).strip())
            last_opt_idx = idx
            continue
        q_lines.append(line)
    if not q_lines:
        q_lines = lines[:1]
    question = ' '.join(_clean_question_prefix(x) if i == 0 else x for i, x in enumerate(q_lines)).strip()
    question = re.sub(r'\s+', ' ', question)
    if len(question) < OCR_MIN_QUESTION_LEN or len(opts) < 2:
        return None
    options_by_letter = {}
    for label, text in opts:
        options_by_letter[label] = re.sub(r'\s+', ' ', str(text or '').strip())
    ordered = [options_by_letter.get(letter, '') for letter in ['A', 'B', 'C', 'D', 'E']]
    if not any(ordered[:2]):
        return None
    payload = {
        'questions': question,
        'option1': ordered[0],
        ...
        'option4': ordered[3],
        'option5': ordered[4],
        'answer': int(answer_idx or 0),
        'explanation': ' '.join([x for x in expl_lines if x]).strip(),
        'type': 'MCQ',
        'section': '',
    }
    return payload
...
def extract_mcqs_from_ocr_text(text: str):
    s = _normalize_ocr_text(text)
    if not s:
        return []
    ...
    seen = set()
    for block in blocks:
        item = _parse_mcq_block(block)
        if not item:
            continue
        key = re.sub(r'\W+', '', (item.get('questions') or '').lower())[:250]
...
def _ocr_extract_mcqs_from_image(path: str):
    text = _ocr_image_text(path)
    return extract_mcqs_from_ocr_text(text)


def _ocr_extract_mcqs_from_pdf(path: str):
    text = _pdf_text_or_ocr(path)
    return extract_mcqs_from_ocr_text(text)


_probaho_original_handle_image = handle_image


async def _extract_and_buffer_items(update: Update, context: ContextTypes.DEFAULT_TYPE, items, *, source_label: str):
    uid = update.effective_user.id if update.effective_user else 0
    added = 0
    for payload in (items or []):
        ...
        added += 1
    if added:
        await ok_html(update, f"{source_label} Processed", f"<code>{h(added)}</code> question(s) extracted.\n\nTotal buffered: <code>{h(buffer_count(uid))}</code>", footer_html="Use <code>/done</code> to export")
        return True
    return False


async def handle_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """FREE OCR-first image extraction. Falls back to original Gemini flow if configured."""
    ensure_user(update)
    if not update.message or not update.effective_user:
        return
    uid = update.effective_user.id
    if is_banned(uid) or not is_private_chat(update) or not can_use_vision(uid) or not vision_mode_on(uid):
        return
    if buffer_count(uid) >= MAX_BUFFERED_QUESTIONS:
        await warn(update, 'Buffer Limit Reached', f'You have {MAX_BUFFERED_QUESTIONS} questions buffered.\n\nUse /done to export or /clear to reset.')
        return
    ...
    if msg.photo:
        ...
        tg_file = await msg.document.get_file()
        name = str(getattr(msg.document, 'file_name', '') or '').lower()
    ...
    try:
        ...
        if _ocr_engine_ready():
            try:
                items = await _run_blocking(_role_of(uid), _ocr_extract_mcqs_from_image, local_path)
            except Exception as e:
                local_error = e
                if OCR_DEBUG:
                    db_log('ERROR', 'ocr_image_failed', {'user_id': uid, 'error': str(e)})
        if await _extract_and_buffer_items(update, context, items, source_label='Image'):
            return
        # Fall back to original Gemini image flow only when OCR did not produce usable MCQs.
        if GEMINI_API_KEY:
            return await _probaho_original_handle_image(update, context)
        msg_txt = 'No MCQs detected from image with free OCR. Try a clearer crop, stronger contrast, or send PDF if the questions span multiple pages.'
        if local_error:
            msg_txt += f'\n\nOCR note: {str(local_error)[:180]}'
        await warn(update, 'No Questions Found', msg_txt)
    finally:
        with contextlib.suppress(Exception):
            os.remove(local_path)
...
async def handle_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Private PDF -> FREE OCR/text extraction -> MCQ buffer."""
    ensure_user(update)
    if not update.message or not update.effective_user:
        return
    uid = update.effective_user.id
    ...
    try:
        if fitz is None:
            await err(update, 'PDF Support Missing', 'PyMuPDF is not installed. Install requirements.txt first.')
            return
        await safe_reply(update, '📄 PDF received. Extracting MCQs...')
        items = await _run_blocking(_role_of(uid), _ocr_extract_mcqs_from_pdf, local_path)
        if await _extract_and_buffer_items(update, context, items, source_label='PDF'):
            return
        await warn(update, 'No Questions Found', 'No MCQs detected in PDF. Try a clearer PDF, smaller page range, or a better scan.')
    except Exception as e:
        db_log('ERROR', 'pdf_extract_failed', {'user_id': uid, 'error': str(e)})
        await err(update, 'PDF Extraction Failed', h(str(e)[:220]))
    finally:
        with contextlib.suppress(Exception):
            os.remove(local_path)


_old_build_app_ocr_patch = build_app


def build_app() -> Application:
    app = _old_build_app_ocr_patch()
    try:
        app.add_handler(MessageHandler(_private_filter(filters.Document.PDF), handle_pdf), group=0)
    except Exception:
        # Some PTB versions may not expose filters.Document.PDF
        app.add_handler(MessageHandler(_private_filter(filters.Document.MimeType('application/pdf')), handle_pdf), group=0)
    return app

# ===== END OCR IMAGE + PDF QUIZ EXTRACTION PATCH =====
